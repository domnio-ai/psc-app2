...
app.get('/api/research/:id/eligible-reviewers',authenticate,async(req,res,next)=>{try{
  ...
  const isResearchManager=['Administrator','Research Manager'].includes(req.user.role)
  const isResearchLead=project.lead_id===req.user.id
  if(!isResearchManager&&!isResearchLead){
    return res.status(403).json({error:'Only the Research Lead or an authorised Research Manager can view eligible reviewers.'})
  }
  ...
  const result=await query(
    `SELECT id,name,email,role,division,status,active
     FROM users
     WHERE active=TRUE
       AND role IN('Reviewer','Research Manager','Administrator')
       AND id<>$1
     ORDER BY
       CASE role
         WHEN 'Reviewer' THEN 1
         WHEN 'Research Manager' THEN 2
...
app.patch('/api/research/:id/reviewers',authenticate,validate(researchReviewerSchema),async(req,res,next)=>{try{
  ...
  const isResearchLead=project.lead_id===req.user.id
  if(!isResearchManager&&!isResearchLead){
    return res.status(403).json({error:'Only the Research Lead or an authorised Research Manager can select formal reviewers.'})
  }

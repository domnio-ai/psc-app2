...
export default function AssignmentReportsPanel({
  ...
  const traceableFinalTasks = useMemo(
    () => finalTasks.filter((task) => taskFinalReportByTaskId.has(task.id)),
            ...
            return (
              <article key={task.id} className={taskStageClass(task)}>
                <span className="source-task-title">
                  ...
                  {repository && <small className="source-repository-title">{repository.title}</small>}
                </span>
                <span>
                ...
                <button
                  type="button"
                  onClick={() => {
                    if (isFinal && repository) onOpenRepository(repository.id);

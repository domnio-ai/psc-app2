...
app.post('/api/assignments/:id/tasks/:taskId/contribution-review',authenticate,validate(assignmentTaskContributionReviewSchema),async(req,res,next)=>{try{
  ...
  if(decision==='Changes Requested'&&!comments)return res.status(400).json({error:'Add clear review comments before requesting changes.'})
  const updated=await transaction(async client=>{
    const row=(await client.query(`UPDATE assignment_tasks SET
      ...
      contribution_integrated_by=CASE WHEN $1='Integrated' THEN $2 ELSE contribution_integrated_by END,
      updated_at=NOW()
      WHERE id=$3 AND assignment_id=$4 RETURNING *`,[
        decision==='Approved'?'Integrated':'Draft',
        req.user.id,
        req.params.taskId,
        req.params.id
      ])).rows[0]
    const action=decision==='Approved'?'TASK_REPORT_APPROVED':'TASK_REPORT_CHANGES_REQUESTED'
    await client.query('INSERT INTO assignment_history(assignment_id,user_id,action,details)VALUES($1,$2,$3,$4)',[
    ...
    if(row.owner_id){
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,$2,$3,'assignment_task',$4)",[
        row.owner_id,
        decision==='Approved'?'Task report approved':'Changes requested on task report',
        decision==='Approved'?`Your task report for “${row.title}” was approved. The assigned reviewer will now generate the final report.`:`Changes were requested on “${row.title}”. ${comments}`,
        row.id
      ])
    }
    ...
    await audit(client,req.user.id,decision==='Approved'?'ASSIGNMENT_TASK_REPORT_APPROVED':'ASSIGNMENT_TASK_REPORT_CHANGES_REQUESTED','assignment_task',row.id,{assignmentId:req.params.id,comments,reportVersion:Number(row.contribution_report_version||0)})
    return row
  })

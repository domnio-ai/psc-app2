...
app.post(
  ...
  async(req,res,next)=>{
    ...
    try{
      ...
      const hasAuthoredContent=existing.some(section=>Boolean(String(section.content||'').trim()))
      ...
      if(!sections.length)return res.status(400).json({error:'The selected template has no report sections.'})

      const existingByKey=new Map(existing.map(section=>[section.section_key,section]))
      ...
      for(let index=0;index<sections.length;index+=1){
        ...
        await client.query(
          `INSERT INTO research_report_sections(
             project_id,section_key,title,content,section_order,status,owner_id,reviewer_id,updated_by,updated_at
           )
           VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9,NOW())
           ON CONFLICT(project_id,section_key) DO UPDATE SET
             title=EXCLUDED.title,
             section_order=EXCLUDED.section_order,
             content=CASE WHEN research_report_sections.content<>'' THEN research_report_sections.content ELSE EXCLUDED.content END,
             status=CASE WHEN research_report_sections.content<>'' THEN research_report_sections.status ELSE EXCLUDED.status END,
          ...
          [
            ...
            current?.owner_id||project.lead_id,
      ...
      await audit(req,{
        action:'RESEARCH_REPORT_TEMPLATE_APPLIED',
      ...
      const refreshed=await query(
        `SELECT
           rrs.id,rrs.section_key,rrs.title,rrs.content,rrs.section_order,rrs.status,
           rrs.owner_id,rrs.reviewer_id,rrs.updated_at,
           u.name updated_by_name,owner.name owner_name,reviewer.name reviewer_name
         FROM research_report_sections rrs
         LEFT JOIN users u ON u.id=rrs.updated_by
         LEFT JOIN users owner ON owner.id=rrs.owner_id
         LEFT JOIN users reviewer ON reviewer.id=rrs.reviewer_id
      ...
      res.json({
        template:{id:template.id,name:template.name,template_key:template.template_key},

...
export default function AssignmentReportSimpleEditor({
  ...
  useEffect(() => {
    // Do not reset a live editor merely because a parent refresh changed updated_at.
    // Reset only when a different report is opened.
    setReport(document);
    setDirty(false);
    setPreview(false);
  }, [document.id]);
            ...
            <section className="simple-report-section" key={section.id}>
              ...
              <div
                ref={(node) => {
                  ...
                  if (
                    node &&
                    node.dataset.app2ReportSectionInitialized !== section.id
                  ) {
                    // Initialize once. The live DOM is the draft source until Save Draft.
                    // React must never write stale section.content back over active typing.
                    node.innerHTML = normalize(section.content || "");
                    node.dataset.app2ReportSectionInitialized = section.id;
                  }
                }}

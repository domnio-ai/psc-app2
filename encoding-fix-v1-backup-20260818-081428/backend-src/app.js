...
const buildTaskContributionReport=({task,assignment,ownerName,preparedBy,override={}})=>{
  const title=plainTaskReportText(override.contributionTitle??task.contribution_title)||`${task.title} Ã¢â‚¬â€ Contribution Report`
  ...
  *{box-sizing:border-box}body{margin:0;background:#eef1f3;color:#1f252a;font-family:Arial,Helvetica,sans-serif}
  ...
  <header class="head"><div class="eyebrow">APP2 Ã‚Â· TASK CONTRIBUTION REPORT</div><h1>${escapeContributionHtml(title)}</h1><div class="sub">${escapeContributionHtml(task.title)}</div></header>
  <section class="meta">
...
if(v.ownerId){
  await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'New task contribution assigned',$2,'assignment_task',$3)",
    [v.ownerId,`You have been assigned Ã¢â‚¬Å“${created.title}Ã¢â‚¬Â for Ã¢â‚¬Å“${assignment?.title||'an assignment'}Ã¢â‚¬Â.${v.expectedContribution?` Expected contribution: ${v.expectedContribution}`:''}`,created.id])
}
if(v.reviewerId){
  await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Task review assigned',$2,'assignment_task',$3)",
    [v.reviewerId,`You are the assigned reviewer for Ã¢â‚¬Å“${created.title}Ã¢â‚¬Â in Ã¢â‚¬Å“${assignment?.title||'an assignment'}Ã¢â‚¬Â. The review becomes actionable when the assignee submits the task contribution.`,created.id])
}
  ...
  try{
    ...
    const result=await transaction(async client=>{
      ...
      if(v.decision==='Rejected'){
        ...
        await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) VALUES($1,'Task request declined',$2,'assignment_task_request',$3)",[request.requested_by,`Your task request Ã¢â‚¬Å“${request.title}Ã¢â‚¬Â was declined${v.comments?`: ${v.comments}`:'.'}`,request.id])
      ...
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) VALUES($1,'Task request approved',$2,'assignment_task_request',$3)",[request.requested_by,`Your task request Ã¢â‚¬Å“${request.title}Ã¢â‚¬Â was approved and converted to task Ã¢â‚¬Å“${task.title}Ã¢â‚¬Â.`,request.id])
      if(taskOwner)await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) VALUES($1,'New task assigned',$2,'assignment_task',$3)",[taskOwner,`You have been assigned Ã¢â‚¬Å“${task.title}Ã¢â‚¬Â.`,task.id])
...
app.patch('/api/assignments/:id/tasks/:taskId/contribution',authenticate,validate(assignmentTaskContributionSchema),async(req,res,next)=>{try{
  ...
  const row=await transaction(async client=>{
    ...
    if(v.contributionStatus==='Ready for Integration'){
      const body=`${req.user.name||current.owner_name||'A team member'} submitted the task report for Ã¢â‚¬Å“${updated.title}Ã¢â‚¬Â in Ã¢â‚¬Å“${assignment?.title||'the assignment'}Ã¢â‚¬Â. Review this submitted version and approve, request changes, or reject it.`
      await client.query(
...
app.post('/api/assignments/:id/tasks/:taskId/contribution-review',authenticate,validate(assignmentTaskContributionReviewSchema),async(req,res,next)=>{try{
  ...
  if(!exactReviewer)return res.status(403).json({error:'Only the reviewer assigned to this task can approve, request changes, or reject this submitted report.'})
  ...
  const updated=await transaction(async client=>{
    ...
    if(row.owner_id){
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,$2,$3,'assignment_task',$4)",[
        ...
        decision==='Approved'
          ? `Your task report for Ã¢â‚¬Å“${row.title}Ã¢â‚¬Â was approved. The assigned reviewer will now generate the final report.`
          : decision==='Rejected'
            ? `Your task report for Ã¢â‚¬Å“${row.title}Ã¢â‚¬Â was rejected. Reason: ${comments}. A manager must reopen the task before further editing.`
            : `Changes were requested on Ã¢â‚¬Å“${row.title}Ã¢â‚¬Â. ${comments}`,
        row.id
      ])
    }
    if(decision==='Rejected'){
      await client.query(`INSERT INTO notifications(user_id,title,body,entity_type,entity_id)
        ...
        WHERE u.active=TRUE AND u.id<>$4 AND (u.role IN('Administrator','Research Manager') OR am.member_role='Lead')`,[
          `Ã¢â‚¬Å“${row.title}Ã¢â‚¬Â was rejected by ${req.user.name||'the assigned reviewer'}. Review the reason and reopen for revision or reassign the task.`,row.id,req.params.id,req.user.id
      ])
    }
    if(decision==='Approved'){
      ...
      for(const userId of finalizers){
        await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Final task report ready to generate',$2,'assignment_task',$3)",[
          userId,`Ã¢â‚¬Å“${row.title}Ã¢â‚¬Â is approved. Generate the final report to complete the task.`,row.id
        ])
      }
    }
...
app.post('/api/assignments/:id/tasks/:taskId/reopen-after-rejection',authenticate,async(req,res,next)=>{try{
  ...
  const row=await transaction(async client=>{
    ...
    if(updated.owner_id)await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Rejected task reopened for revision',$2,'assignment_task',$3)",[updated.owner_id,`Ã¢â‚¬Å“${updated.title}Ã¢â‚¬Â has been reopened for revision. Update the draft, preview it, and resubmit to the assigned reviewer.`,updated.id])
...
app.post('/api/assignments/:id/tasks/:taskId/contribution-finalize',authenticate,async(req,res,next)=>{let storedFinalPath;try{
  ...
  const repositoryTitle=plainTaskReportText(current.contribution_title)||`${current.title} Ã¢â‚¬â€ Final Task Report`
  ...
  const result=await transaction(async client=>{
    ...
    const description=`Final approved task report for Ã¢â‚¬Å“${current.title}Ã¢â‚¬Â under assignment Ã¢â‚¬Å“${current.assignment_title}Ã¢â‚¬Â. Prepared by ${current.owner_name||'the task owner'} and approved by ${reviewerName}.`
    ...
    if(row.owner_id){
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Task completed',$2,'assignment_task',$3)",[
        row.owner_id,`The final approved report for Ã¢â‚¬Å“${row.title}Ã¢â‚¬Â has been generated, published to the Document Repository, and the task is complete.`,row.id
      ])
    }
...
app.delete('/api/assignments/:id/tasks/:taskId',authenticate,authorize('Administrator','Research Manager'),validate(z.object({reason:z.string().trim().min(10).max(1000)})),async(req,res,next)=>{try{
  const result=await transaction(async client=>{
    ...
    if(task.owner_id&&task.owner_id!==req.user.id){
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Task deleted',$2,'assignment',$3)",[task.owner_id,`The task Ã¢â‚¬Å“${task.title}Ã¢â‚¬Â was deleted by ${req.user.name||'an authorised manager'}. Reason: ${reason}`,req.params.id])
    }
...
app.post('/api/assignments/:id/reviews',authenticate,validate(z.object({decision:z.enum(['Submitted','Under Review','Changes Requested','Approved']),comments:z.string().max(4000).default('')})),async(req,res,next)=>{
  try{
    ...
    const row=await transaction(async client=>{
      ...
      if(!allowed[req.validated.decision].includes(latest))throw Object.assign(new Error(`The Ã¢â‚¬Å“${req.validated.decision}Ã¢â‚¬Â action is not valid after ${latest||'no submission'}. Refresh the Review tab.`),{status:409})
...
const reportPlainText=value=>String(value||'').replace(/<br\s*\/?>/gi,'\n').replace(/<\/(td|th)>/gi,'\t').replace(/<\/tr>/gi,'\n').replace(/<\/p>|<\/li>|<\/h[1-6]>/gi,'\n').replace(/<li[^>]*>/gi,'Ã¢â‚¬Â¢ ').replace(/<[^>]+>/g,'').replace(/&nbsp;/gi,' ').replace(/&amp;/gi,'&').replace(/&lt;/gi,'<').replace(/&gt;/gi,'>').replace(/&quot;/gi,'"').replace(/&#39;/gi,"'").replace(/[ \t]+\n/g,'\n').replace(/\n{3,}/g,'\n\n').trim()
...
const compileAssignmentSectionContent=({section,tasks,assignment,knowledge,reportType})=>{
  ...
  const taskTitles=tasks.map(task=>`<li>${escapeHtml(task.contribution_title||task.title)} Ã¢â‚¬â€ ${escapeHtml(task.owner_name||'Task owner')}</li>`).join('')
  ...
  if(key==='references'){const taskRefs=tasks.map(task=>`<li>Final Task Report: ${escapeHtml(task.contribution_title||task.title)} Ã¢â‚¬â€ ${escapeHtml(task.owner_name||'Task owner')}</li>`).join('');const repoRefs=knowledge.map(item=>`<li>Repository evidence: ${escapeHtml(item.title)} Ã‚Â· ${escapeHtml(item.category||'Document')}</li>`).join('');return taskRefs||repoRefs?`<ul>${taskRefs}${repoRefs}</ul>`:''}
  if(key==='appendices'){const taskRefs=tasks.map((task,index)=>`<li>Appendix ${index+1}: Final Task Report Ã¢â‚¬â€ ${escapeHtml(task.contribution_title||task.title)} (${escapeHtml(task.owner_name||'Task owner')})</li>`).join('');const repoRefs=knowledge.map((item,index)=>`<li>Supporting Evidence ${index+1}: ${escapeHtml(item.title)} Ã‚Â· ${escapeHtml(item.category||'Document')}</li>`).join('');return taskRefs||repoRefs?`<p>This source register records the approved inputs used to compile the Final Assignment Report.</p><ul>${taskRefs}${repoRefs}</ul>`:'<p>No appendices or supporting sources have been registered.</p>'}
...
app.post('/api/assignment-reports/:id/submit',authenticate,validate(z.object({reviewerId:z.string().uuid(),reviewDueDate:z.string().nullable().default(null),comments:z.string().max(4000).default('')})),async(req,res,next)=>{try{
  ...
  if(!allowed||!['Lead','Manager'].includes(role))return res.status(403).json({error:'Only the Assignment Lead or an authorised manager can submit the report.'})
  ...
  const reviewer=(await query('SELECT id,name,active FROM users WHERE id=$1',[req.validated.reviewerId])).rows[0]
  ...
  const result=await transaction(async client=>{
    const sections=(await client.query('SELECT section_key,title,section_order,content,completion,section_status,owner_id FROM generated_document_sections WHERE document_id=$1 ORDER BY section_order',[document.id])).rows
    if(!sections.length||sections.some(item=>!['Ready','Complete'].includes(item.section_status)))throw Object.assign(new Error('Mark every required report section Ready before submission.'),{status:409})
    await client.query("INSERT INTO generated_document_versions(document_id,version_number,sections,change_note,status,template_version,created_by)VALUES($1,$2,$3,$4,'Submitted',$5,$6)ON CONFLICT(document_id,version_number)DO NOTHING",[document.id,document.version,JSON.stringify(sections),req.validated.comments,document.template_version,req.user.id])
    const updated=(await client.query("UPDATE generated_documents SET status='Submitted',reviewer_id=$1,review_due_date=$2,submitted_by=$3,submitted_at=NOW(),updated_at=NOW() WHERE id=$4 RETURNING *",[reviewer.id,req.validated.reviewDueDate,req.user.id,document.id])).rows[0]
    await client.query("INSERT INTO generated_document_reviews(document_id,version_number,reviewer_id,decision,comments)VALUES($1,$2,$3,'Submitted',$4)",[document.id,document.version,reviewer.id,req.validated.comments])
    await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Assignment report review assigned',$2,'generated_document',$3)",[reviewer.id,`Review Ã¢â‚¬Å“${document.title}Ã¢â‚¬Â for assignment Ã¢â‚¬Å“${document.context_id}Ã¢â‚¬Â.`,document.id])
    await audit(client,req.user.id,'ASSIGNMENT_REPORT_SUBMITTED','generated_document',document.id,{version:document.version,reviewerId:reviewer.id,reviewerName:reviewer.name})
    return updated
  })
  res.json(result)
}catch(error){if(error.status)return res.status(error.status).json({error:error.message});next(error)}})
app.post('/api/assignments/:id/reports/compile',authenticate,validate(z.object({templateId:z.string().uuid(),title:z.string().min(3).max(300),taskIds:z.array(z.string().uuid()).min(1).max(100),knowledgeIds:z.array(z.string().uuid()).max(100).default([]),reportType:z.enum(['Progress','Final']).default('Progress')})),async(req,res,next)=>{try{
  ...
  const allTasks=(await query("SELECT t.*,u.name owner_name FROM assignment_tasks t LEFT JOIN users u ON u.id=t.owner_id WHERE t.assignment_id=$1 AND t.archived_at IS NULL ORDER BY t.created_at",[assignment.id])).rows
  ...
  const result=await transaction(async client=>{
    ...
    await audit(client,req.user.id,'ASSIGNMENT_REPORT_COMPILED','generated_document',document.id,{assignmentId:assignment.id,reportType:req.validated.reportType,taskIds:selectedIds,knowledgeIds:req.validated.knowledgeIds})
    return document
  })
...
app.post('/api/assignment-reports/:id/review',authenticate,validate(z.object({decision:z.enum(['Under Review','Changes Requested','Approved']),comments:z.string().max(4000).default('')})),async(req,res,next)=>{try{
  ...
  if(req.validated.decision==='Changes Requested'&&!req.validated.comments.trim())return res.status(400).json({error:'Explain the changes required.'})
  const updated=await transaction(async client=>{
    const row=(await client.query("UPDATE generated_documents SET status=$1,version=CASE WHEN $1='Changes Requested' THEN version+1 ELSE version END,approved_by=CASE WHEN $1='Approved' THEN $2 ELSE approved_by END,approved_at=CASE WHEN $1='Approved' THEN NOW() ELSE approved_at END,updated_at=NOW() WHERE id=$3 RETURNING *",[req.validated.decision,req.user.id,document.id])).rows[0]
    if(req.validated.decision==='Changes Requested')await client.query("UPDATE generated_document_sections SET section_status='Needs Changes',completion=LEAST(completion,90) WHERE document_id=$1 AND section_status IN('Ready','Complete')",[document.id])
    await client.query('INSERT INTO generated_document_reviews(document_id,version_number,reviewer_id,decision,comments)VALUES($1,$2,$3,$4,$5)',[document.id,document.version,req.user.id,req.validated.decision,req.validated.comments])
    await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,$2,$3,'generated_document',$4)",[document.created_by,req.validated.decision==='Changes Requested'?'Assignment report changes requested':'Assignment report approved',`Ã¢â‚¬Å“${document.title}Ã¢â‚¬Â is now ${req.validated.decision}. ${req.validated.comments}`,'generated_document',document.id])
...
app.post('/api/assignment-reports/:id/finalize',authenticate,async(req,res,next)=>{let storedFinalPath;try{
  const document=(await query(`SELECT d.*,t.name template_name,creator.name created_by_name,reviewer.name reviewer_name,a.title assignment_title,a.division assignment_division,a.created_at assignment_created_at,a.due_date assignment_due_date,lead.name lead_name,(SELECT string_agg(team_user.name, ', ' ORDER BY CASE WHEN team_member.member_role='Lead' THEN 0 ELSE 1 END,team_user.name) FROM assignment_members team_member JOIN users team_user ON team_user.id=team_member.user_id WHERE team_member.assignment_id=a.id) team_names FROM generated_documents d JOIN document_templates t ON t.id=d.template_id JOIN users creator ON creator.id=d.created_by LEFT JOIN users reviewer ON reviewer.id=d.reviewer_id JOIN assignments a ON a.id=d.context_id LEFT JOIN assignment_members am ON am.assignment_id=a.id AND am.member_role='Lead' LEFT JOIN users lead ON lead.id=am.user_id WHERE d.id=$1 AND d.context='Assignment' LIMIT 1`,[req.params.id])).rows[0]
  if(!document)return res.status(404).json({error:'Assignment report not found.'})
  if(document.created_by===req.user.id)return res.status(403).json({error:'The report author cannot generate the approved final report.'})
  if(document.reviewer_id!==req.user.id)return res.status(403).json({error:'Only the assigned reviewer can generate the final assignment report.'})
  if(document.status==='Final'){
    const existing=(await query(`SELECT k.id,k.title FROM repository_entity_links rel JOIN knowledge_items k ON k.id=rel.knowledge_id WHERE rel.entity_type='report' AND rel.entity_id=$1 AND k.status='Published' ORDER BY k.created_at DESC LIMIT 1`,[document.id])).rows[0]
  ...
  const tasks=(await query("SELECT id FROM assignment_tasks WHERE assignment_id=$1 AND archived_at IS NULL AND contribution_status<>'Accepted'",[document.context_id])).rows
  ...
  const sections=(await query('SELECT title,content,section_key FROM generated_document_sections WHERE document_id=$1 ORDER BY section_order',[document.id])).rows
  ...
  const repositoryTitle=document.title.replace(/\s*[Ã¢â‚¬â€-]\s*Draft Assignment Report$/i,'').trim()||`${document.assignment_title} Ã¢â‚¬â€ Final Assignment Report`
  const finalSections=[...sections.filter(section=>section.section_key!=='review_approval'),{title:'Review and Approval Record',content:`Status: FINAL Ã¢â‚¬â€ APPROVED
Reviewed and approved by: ${document.reviewer_name||req.user.name}
Approval date: ${approvalTime}
Controlled report version: ${document.version}`}]
  ...
  const finalDocx=await buildDocx(repositoryTitle,{Organization:'Public Service Commission','Report Type':'Final Assignment Report','Assignment Reference':assignmentReference,'Assignment Title':document.assignment_title,Division:document.assignment_division||'Not specified','Assignment Lead':document.lead_name||'Not specified','Team Members':document.team_names||document.lead_name||'Not specified','Reporting Period':`${reportingStart} Ã¢â‚¬â€œ ${reportingEnd}`,'Report Reference':document.reference,'Controlled Version':String(document.version),Classification:document.classification||'Official','Prepared / Compiled by':document.created_by_name,'Reviewed and Approved by':document.reviewer_name||req.user.name,'Approval Date':approvalTime,Status:'FINAL Ã¢â‚¬â€ APPROVED'},finalSections)
  ...
  const originalName=`${safeBase}-final-v${document.version}.docx`
  const stored=await storeDocument(finalDocx,'.docx');storedFinalPath=stored.absolutePath
  const sha256=crypto.createHash('sha256').update(finalDocx).digest('hex')
  const mimeType='application/vnd.openxmlformats-officedocument.wordprocessingml.document'
  const result=await transaction(async client=>{
    const locked=(await client.query('SELECT status FROM generated_documents WHERE id=$1 FOR UPDATE',[document.id])).rows[0]
    ...
    const category=(await client.query("SELECT id FROM document_categories WHERE name='Assignment Reports' LIMIT 1")).rows[0]||null
    const description=`Final approved assignment report for Ã¢â‚¬Å“${document.assignment_title}Ã¢â‚¬Â. Prepared by ${document.created_by_name} and approved by ${document.reviewer_name||req.user.name}.`
    const tags=['final-report','assignment-report','approved']
    const item=(await client.query(`INSERT INTO knowledge_items(title,description,category,category_id,tags,author,document_date,status,created_by,approved_by,approved_at,current_version,source_type,source_url,directorate,document_type,subject,classification,felix_enabled,reviewed_by,reviewed_at)VALUES($1,$2,'Assignment Reports',$3,$4,$5,CURRENT_DATE,'Published',$6,$6,NOW(),1,'Assignment','',$7,'Assignment Final Report',$8,'INTERNAL',TRUE,$6,NOW()) RETURNING *`,[repositoryTitle,description,category?.id||null,tags,document.lead_name||document.created_by_name,req.user.id,document.assignment_division||null,document.assignment_title])).rows[0]
    const version=(await client.query(`INSERT INTO knowledge_versions(knowledge_id,version_number,uploader_id,original_name,stored_name,storage_path,mime_type,size_bytes,sha256_hash,is_current,approved_by,approved_at,notes)VALUES($1,1,$2,$3,$4,$5,$6,$7,$8,TRUE,$2,NOW(),'Automatically generated from reviewer-approved Final Assignment Report') RETURNING *`,[item.id,req.user.id,originalName,stored.storedName,stored.storagePath,mimeType,finalDocx.length,sha256])).rows[0]
    for(const tag of tags){const tagRow=(await client.query('INSERT INTO document_tags(name,normalized_name,created_by)VALUES($1,lower($1),$2)ON CONFLICT(normalized_name)DO UPDATE SET name=EXCLUDED.name RETURNING id',[tag,req.user.id])).rows[0];await client.query('INSERT INTO document_tag_links(knowledge_id,tag_id)VALUES($1,$2)ON CONFLICT DO NOTHING',[item.id,tagRow.id])}
    await client.query("INSERT INTO repository_entity_links(knowledge_id,entity_type,entity_id,linked_by)VALUES($1,'report',$2,$3)ON CONFLICT DO NOTHING",[item.id,document.id,req.user.id])
    await client.query("INSERT INTO repository_entity_links(knowledge_id,entity_type,entity_id,linked_by)VALUES($1,'assignment',$2,$3)ON CONFLICT DO NOTHING",[item.id,document.context_id,req.user.id])
    await client.query('INSERT INTO knowledge_assignment_links(knowledge_id,assignment_id,linked_by)VALUES($1,$2,$3)ON CONFLICT DO NOTHING',[item.id,document.context_id,req.user.id])
    const final=(await client.query("UPDATE generated_documents SET status='Final',updated_at=NOW() WHERE id=$1 RETURNING *",[document.id])).rows[0]
    await client.query("UPDATE assignments SET status='Completed',updated_at=NOW() WHERE id=$1",[document.context_id])
    await client.query('INSERT INTO assignment_history(assignment_id,user_id,action,details)VALUES($1,$2,$3,$4)',[document.context_id,req.user.id,'ASSIGNMENT_FINAL_REPORT_GENERATED',JSON.stringify({generatedDocumentId:document.id,repositoryDocumentId:item.id,repositoryVersionId:version.id,version:document.version})])
    await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Final Assignment Report published',$2,'generated_document',$3)",[document.created_by,`The Final Assignment Report for Ã¢â‚¬Å“${document.assignment_title}Ã¢â‚¬Â has been generated, published to the Document Repository, and the assignment is complete.`,document.id])
    await audit(client,req.user.id,'ASSIGNMENT_FINAL_REPORT_GENERATED','generated_document',document.id,{assignmentId:document.context_id,repositoryDocumentId:item.id,version:document.version})
...
app.post('/api/document-reviews/:id/decision',authenticate,authorize('Administrator','Research Manager','Reviewer'),validate(z.object({approved:z.boolean(),comments:z.string().max(2000).default('')})),async(req,res,next)=>{
  try{
    ...
    const result=await transaction(async client=>{
      ...
      await client.query('INSERT INTO notifications(user_id,title,body,entity_type,entity_id) VALUES($1,$2,$3,$4,$5)',[item.created_by,req.validated.approved?'Document approved':'Document returned for correction',req.validated.approved?`Ã¢â‚¬Å“${item.title}Ã¢â‚¬Â has been approved and published.`:`Ã¢â‚¬Å“${item.title}Ã¢â‚¬Â was rejected: ${req.validated.comments}`,'knowledge',item.id])
...
app.get('/api/notifications',authenticate,async(req,res,next)=>{
  try{
    await transaction(async client=>{
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) SELECT am.user_id,'Assigned task','You are assigned to Ã¢â‚¬Å“'||a.title||'Ã¢â‚¬Â ('||a.status||')'||CASE WHEN a.due_date IS NOT NULL THEN ', due '||TO_CHAR(a.due_date,'DD Mon YYYY') ELSE '' END||'.','assignment',a.id FROM assignments a JOIN assignment_members am ON am.assignment_id=a.id WHERE am.user_id=$1 AND NOT EXISTS(SELECT 1 FROM notifications n WHERE n.user_id=am.user_id AND n.entity_id=a.id::text AND n.title IN('Assigned task','New assignment','Assignment allocated'))",[req.user.id])
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) SELECT am.user_id,'Assignment status update','Ã¢â‚¬Å“'||a.title||'Ã¢â‚¬Â is now '||a.status||'.','assignment_status',a.id FROM assignments a JOIN assignment_members am ON am.assignment_id=a.id WHERE am.user_id=$1 AND NOT EXISTS(SELECT 1 FROM notifications n WHERE n.user_id=am.user_id AND n.entity_id=a.id::text AND n.title='Assignment status update' AND n.body='Ã¢â‚¬Å“'||a.title||'Ã¢â‚¬Â is now '||a.status||'.')",[req.user.id])
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) SELECT am.user_id,'Assignment completed','Ã¢â‚¬Å“'||a.title||'Ã¢â‚¬Â has been completed and retained in your activity history.','assignment_completed',a.id FROM assignments a JOIN assignment_members am ON am.assignment_id=a.id WHERE am.user_id=$1 AND a.status='Completed' AND NOT EXISTS(SELECT 1 FROM notifications n WHERE n.user_id=am.user_id AND n.entity_id=a.id::text AND n.title='Assignment completed')",[req.user.id])
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) SELECT am.user_id,'Assignment due soon','Ã¢â‚¬Å“'||a.title||'Ã¢â‚¬Â is due on '||TO_CHAR(a.due_date,'DD Mon YYYY')||'. Please review and update its progress.','assignment_due',a.id FROM assignments a JOIN assignment_members am ON am.assignment_id=a.id WHERE am.user_id=$1 AND a.status NOT IN('Completed','Overdue') AND a.due_date BETWEEN CURRENT_DATE AND CURRENT_DATE+2 AND NOT EXISTS(SELECT 1 FROM notifications n WHERE n.user_id=am.user_id AND n.entity_id=a.id::text AND n.title='Assignment due soon' AND n.body LIKE '%'||TO_CHAR(a.due_date,'DD Mon YYYY')||'%')",[req.user.id])
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) SELECT am.user_id,'Assignment overdue','Ã¢â‚¬Å“'||a.title||'Ã¢â‚¬Â was due on '||TO_CHAR(a.due_date,'DD Mon YYYY')||'. Immediate action is required.','assignment_overdue',a.id FROM assignments a JOIN assignment_members am ON am.assignment_id=a.id WHERE am.user_id=$1 AND a.status!='Completed' AND a.due_date<CURRENT_DATE AND NOT EXISTS(SELECT 1 FROM notifications n WHERE n.user_id=am.user_id AND n.entity_id=a.id::text AND n.title='Assignment overdue' AND n.body LIKE '%'||TO_CHAR(a.due_date,'DD Mon YYYY')||'%')",[req.user.id])
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) SELECT $1,'Previously approved notice','Ã¢â‚¬Å“'||a.title||'Ã¢â‚¬Â was approved and published on the Notice Board.','notice',a.id FROM alerts a WHERE a.status='Published' AND(a.audience_role IS NULL OR a.audience_role=$2) AND NOT EXISTS(SELECT 1 FROM notifications n WHERE n.user_id=$1 AND n.entity_type='notice' AND n.entity_id=a.id::text)",[req.user.id,req.user.role])
    })

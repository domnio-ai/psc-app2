...
foreach ($Contract in $Manifest.source_contracts) {
    $File = Join-Path $App2Root ($Contract.file -replace '/', '\')
    if (-not (Test-Path $File)) {
        throw "Protected contract $($Contract.id) failed: file missing $($Contract.file)"
    }
    ...
    if ($Contract.required_all) {
        foreach ($Pattern in $Contract.required_all) {
            if (-not $Content.Contains([string]$Pattern)) {
                throw "Protected contract $($Contract.id) failed: missing required signature '$Pattern' in $($Contract.file)"
            }
        }
    }
    ...
    if ($Contract.required_any) {
        ...
        if (-not $Found) {
            throw "Protected contract $($Contract.id) failed: none of its accepted signatures exist in $($Contract.file)"
        }
    }
...
$RequiredTables = @($Manifest.protected_database.required_tables)
...
if (-not $SkipDataCountFloor) {
    foreach ($Name in @("users","assignments","research_projects","documents")) {
        ...
        if ($CurrentValue -lt $BaselineValue) {
            throw "Protected data floor failed for $Name. Baseline=$BaselineValue Current=$CurrentValue. Do not create a new checkpoint unless this decrease was intentional and verified."
        }
    }
}

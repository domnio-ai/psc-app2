...
app.post('/api/research/:id/report/generate',authenticate,validate(z.object({
  ...
  }else if(!isManager&&!isAssignedReviewer){
    return res.status(403).json({error:'Only the assigned research reviewer or an authorised research manager can generate the approved final report.'})
  }
  ...
  if(!draftMode&&sections.some(section=>section.status!=='Approved')){
    return res.status(409).json({error:'Approve every required research report section before generating the final report.'})
  }
  ...
  if(!draftMode&&!isManager&&approvedVersion.reviewer_id!==req.user.id){
    return res.status(403).json({error:'Only the reviewer who approved this submitted version can generate its final report.'})
  }

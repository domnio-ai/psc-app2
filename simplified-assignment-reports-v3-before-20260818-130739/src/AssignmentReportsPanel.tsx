...
export default function AssignmentReportsPanel({
  ...
  const primaryOpenReport =
    openReports.find((item) => item.template_key === "assignment-final-report") ||
    openReports[0] ||
    null;
  const secondaryOpenReports = primaryOpenReport
    ? openReports.filter((item) => item.id !== primaryOpenReport.id)
    : [];
  const visibleOpenReports = primaryOpenReport
    ? [primaryOpenReport, ...(showOtherReports ? secondaryOpenReports : [])]
    : [];
  ...
  return (
    <section className="assignment-reports-panel assignment-reports-v3">
      ...
      <section className="assignment-report-workflow-list">
        ...
        <div className="assignment-report-workflow-guide" aria-label="Assignment report workflow">
          <span>1. Edit sections</span>
          <span>2. Mark all Ready</span>
          <span>3. Preview & submit</span>
          <span>4. Reviewer approves</span>
          <span>5. Generate final</span>
        </div>
        ...
        {visibleOpenReports.map((report) => {
          ...
          const hasExternalReport = Boolean(report.external_import_id);
          const submissionReady = sectionsReady || hasExternalReport;
          const sectionsRemaining = Math.max(total - ready, 0);
          ...
          return (
            <article key={report.id} className={`assignment-report-workflow-card ${report.id === primaryOpenReport?.id ? "is-primary-report" : "is-secondary-report"}`}>
              <header>
                <div>
              ...
              <div className="assignment-report-workflow-meta">
                ...
                {hasExternalReport && (
                  <span><small>Review source</small><strong>Imported {report.external_import_name}</strong></span>
                )}
              </div>
              <div className="assignment-report-next-action">
                <div>
                  <strong>Next action</strong>
                  <span>{report.status === "Approved" ? "Generate and publish the approved final report." : submissionReady ? (hasExternalReport ? "Open the imported report, assign a reviewer and submit the preserved file for formal review." : "Preview the completed draft, assign a reviewer and submit it.") : `Complete and mark ${sectionsRemaining} remaining section${sectionsRemaining === 1 ? "" : "s"} Ready, or import a complete PDF/DOCX report.`}</span>
                </div>
                <b>{hasExternalReport ? "External report ready" : `${ready}/${total} Ready`}</b>
              </div>
              <footer>
                <div className="assignment-report-stage-actions">
                  <button type="button" className="app-action-primary" onClick={() => onOpen(report.id)}>{report.id === primaryOpenReport?.id ? "Continue Working" : "Open Report"}</button>
                  <button type="button" onClick={() => onPreview(report.id)}>Preview Draft</button>
                </div>
                ...
                {canManage && ["Draft", "Revised", "Changes Requested"].includes(report.status) && (
                  <div className="assignment-report-submit-controls">
                    ...
                    <button
                      type="button"
                      className="primary"
                      disabled={!submissionReady || !reviewerId || busy === `submit-${report.id}`}
                      title={!submissionReady ? "Mark every report section Ready, or import a complete PDF/DOCX report." : "Submit the controlled report version to the assigned reviewer."}
                      onClick={() =>
                    ...
                    {!submissionReady && (
                      <small className="assignment-report-submit-blocker">Mark all {total} sections Ready ({sectionsRemaining} remaining), or import a complete PDF/DOCX report for review.</small>
                    )}
                  </div>
                )}
                ...
                {(assignedReviewer || canManage) && report.status === "Approved" && report.template_key === "assignment-final-report" && (
                  <button
                    type="button"
                    className="primary"
        ...
        {secondaryOpenReports.length > 0 && (
          <button
            type="button"
            className="assignment-report-other-toggle"
            onClick={() => setShowOtherReports((current) => !current)}
          >
            {showOtherReports
              ? "Hide other reports"
              : `Other reports (${secondaryOpenReports.length})`}
          </button>
        )}
      ...
      <section className="report-document-group final-outputs">
        ...
        {finalReports.map((item) => (
          <article key={item.id}>
            ...
            <b>Final</b>
            <button type="button" className="app-action-button" onClick={() => onPreview(item.id)}>Preview Final Report</button>
          </article>
        ))}

...
app.get('/api/assignments',authenticate,async(req,res,next)=>{try{const all=['Administrator','Research Manager'].includes(req.user.role);const result=await query(`SELECT a.*,creator.name created_by_name,ral.project_id research_id,rp.title research_title,ral.relation_type research_relation_type,
COALESCE(json_agg(json_build_object('id',u.id,'name',u.name,'role',am.member_role)) FILTER(WHERE u.id IS NOT NULL),'[]') members,
COALESCE(task_state.total_tasks,0)::int task_count,
COALESCE(task_state.completed_tasks,0)::int completed_task_count,
COALESCE(task_state.accepted_reports,0)::int accepted_task_report_count,
final_report.status final_report_status,
CASE
  WHEN COALESCE(task_state.total_tasks,0)=0 THEN 'Planning'
  WHEN COALESCE(task_state.completed_tasks,0)<COALESCE(task_state.total_tasks,0) THEN 'In Progress'
  WHEN COALESCE(task_state.accepted_reports,0)<COALESCE(task_state.total_tasks,0) THEN 'Task Reporting'
  WHEN final_report.id IS NULL THEN 'Ready for Final Report'
  WHEN final_report.status='Changes Requested' THEN 'Changes Requested'
  WHEN final_report.status IN('Submitted','Under Review') THEN 'Final Report Review'
  WHEN final_report.status='Approved' THEN 'Approved · Finalization Pending'
  WHEN final_report.status='Final' THEN 'Completed'
  ELSE 'Final Report Draft'
END workflow_status
FROM assignments a
JOIN users creator ON creator.id=a.created_by
LEFT JOIN assignment_members am ON am.assignment_id=a.id
LEFT JOIN users u ON u.id=am.user_id
LEFT JOIN research_assignment_links ral ON ral.assignment_id=a.id
LEFT JOIN research_projects rp ON rp.id=ral.project_id
LEFT JOIN LATERAL(
  SELECT
    COUNT(*) FILTER(WHERE t.archived_at IS NULL)::int total_tasks,
    COUNT(*) FILTER(WHERE t.archived_at IS NULL AND t.status='Completed')::int completed_tasks,
    COUNT(*) FILTER(WHERE t.archived_at IS NULL AND t.contribution_status='Accepted')::int accepted_reports
  FROM assignment_tasks t
  WHERE t.assignment_id=a.id
) task_state ON TRUE
LEFT JOIN LATERAL(
  SELECT gd.id,gd.status
  FROM generated_documents gd
  WHERE gd.context='Assignment'
    AND gd.context_id=a.id
    AND gd.template_key='assignment-final-report'
  ORDER BY gd.updated_at DESC,gd.created_at DESC
  LIMIT 1
) final_report ON TRUE
WHERE $1::boolean OR EXISTS(SELECT 1 FROM assignment_members mine WHERE mine.assignment_id=a.id AND mine.user_id=$2)
GROUP BY a.id,creator.name,ral.project_id,rp.title,ral.relation_type,task_state.total_tasks,task_state.completed_tasks,task_state.accepted_reports,final_report.id,final_report.status
ORDER BY a.created_at DESC`,[all,req.user.id]);res.json(result.rows)}catch(error){next(error)}})
...
app.delete('/api/assignments/:id/tasks/:taskId',authenticate,authorize('Administrator','Research Manager'),validate(z.object({reason:z.string().trim().min(10).max(1000)})),async(req,res,next)=>{try{
  const result=await transaction(async client=>{
    const task=(await client.query(`SELECT t.*,
    EXISTS(
      SELECT 1 FROM repository_entity_links rel
      WHERE rel.entity_type='task' AND rel.entity_id=t.id
    ) has_repository_record,
    EXISTS(
      SELECT 1 FROM assignment_report_sources ars
      WHERE ars.task_id=t.id
    ) has_assignment_report_source
    FROM assignment_tasks t WHERE t.id=$1 AND t.assignment_id=$2 FOR UPDATE`,[req.params.taskId,req.params.id])).rows[0]
    if(!task)return null
    if(task.status==='Completed'||task.contribution_status!=='Draft'||task.has_repository_record||task.has_assignment_report_source){
      throw Object.assign(new Error('This task is already part of the formal assignment reporting record and cannot be deleted. Archive it instead so report provenance is preserved.'),{statusCode:409})
    }
  ...
  try{
    const created=await transaction(async client=>{
      ...
      if(reviewerIds.length){
        const valid=(await client.query("SELECT id FROM users WHERE id=ANY($1::uuid[]) AND active=TRUE AND role IN('Research Officer','Reviewer','Research Manager','Administrator')",[reviewerIds])).rows.map(row=>row.id)
        if(valid.length!==reviewerIds.length)throw Object.assign(new Error('Choose only active eligible reviewers. Research Officers may serve as contextual reviewers when formally assigned.'),{statusCode:400})
      }
...
app.patch('/api/research/:id/plan',authenticate,validate(researchPlanSchema),async(req,res,next)=>{try{
  ...
  const canEdit=['Administrator','Research Manager'].includes(req.user.role)
  if(!canEdit)return res.status(403).json({error:'Only an authorised research manager can edit the Research Brief. The research lead may submit a change request.'})
  const v=req.validated
...
const researchBriefChangeRequestSchema=z.object({reason:z.string().trim().min(5).max(2000)})
app.post('/api/research/:id/brief-change-request',authenticate,validate(researchBriefChangeRequestSchema),async(req,res,next)=>{try{
  const project=(await query('SELECT id,title,lead_id FROM research_projects WHERE id=$1',[req.params.id])).rows[0]
  if(!project)return res.status(404).json({error:'Research project not found.'})
  if(project.lead_id!==req.user.id)return res.status(403).json({error:'Only the research lead may request a change to the Research Brief.'})
  const reason=req.validated.reason.trim()
  await transaction(async client=>{
    await client.query('INSERT INTO research_activity(project_id,user_id,action,details)VALUES($1,$2,$3,$4)',[project.id,req.user.id,'RESEARCH_BRIEF_CHANGE_REQUESTED',JSON.stringify({reason})])
    const managers=(await client.query("SELECT id FROM users WHERE active=TRUE AND role IN('Administrator','Research Manager')")).rows
    for(const manager of managers)await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Research Brief change requested',$2,'research',$3)",[manager.id,`A change to the Research Brief for “${project.title}” has been requested: ${reason}`,project.id])
    await audit(client,req.user.id,'RESEARCH_BRIEF_CHANGE_REQUESTED','research_project',project.id,{reason})
  })
...
app.patch('/api/research/:id/team',authenticate,validate(researchTeamSchema),async(req,res,next)=>{try{
  const project=(await query('SELECT * FROM research_projects WHERE id=$1',[req.params.id])).rows[0]
  if(!project)return res.status(404).json({error:'Research project not found.'})
  const isResearchManager=['Administrator','Research Manager'].includes(req.user.role)
  const isResearchLead=project.lead_id===req.user.id
  if(!isResearchManager&&!isResearchLead)return res.status(403).json({error:'Only the research lead or an authorised research manager can update the project team.'})
  const v=req.validated
  if(!isResearchManager&&v.leadId!==project.lead_id)return res.status(403).json({error:'The research lead may add or remove team members but cannot transfer the lead role.'})
...
app.get('/api/research/:id/eligible-reviewers',authenticate,async(req,res,next)=>{try{
  const project=(await query(
    'SELECT id,lead_id FROM research_projects WHERE id=$1',
    [req.params.id]
  )).rows[0]
  if(!project)return res.status(404).json({error:'Research project not found.'})
  ...
  const isResearchManager=['Administrator','Research Manager'].includes(req.user.role)
  const isResearchLead=project.lead_id===req.user.id
  const isReportBuilder=(await query(
    "SELECT 1 FROM research_collaborators WHERE project_id=$1 AND user_id=$2 AND COALESCE(role,'Researcher')<>'Reviewer'",
    [project.id,req.user.id]
  )).rowCount>0
  if(!isResearchManager&&!isResearchLead&&!isReportBuilder){
    return res.status(403).json({error:'Only the Research Lead, an active report builder, or an authorised Research Manager can view eligible reviewers.'})
  }

  const result=await query(
    `SELECT id,name,email,role,division,status,active
     FROM users
     WHERE active=TRUE
       AND role IN('Research Officer','Reviewer','Research Manager','Administrator')
       AND id<>$1
       AND id<>$2
     ORDER BY
       CASE role
         WHEN 'Research Officer' THEN 1
         WHEN 'Reviewer' THEN 2
         WHEN 'Research Manager' THEN 3
...
app.patch('/api/research/:id/reviewers',authenticate,validate(researchReviewerSchema),async(req,res,next)=>{try{
  const project=(await query('SELECT * FROM research_projects WHERE id=$1',[req.params.id])).rows[0]
  if(!project)return res.status(404).json({error:'Research project not found.'})
  const isResearchManager=['Administrator','Research Manager'].includes(req.user.role)
  const isResearchLead=project.lead_id===req.user.id
  ...
  if(!isResearchManager&&!isResearchLead&&!isReportBuilder){
    return res.status(403).json({error:'Only the Research Lead, an active report builder, or an authorised Research Manager can select formal reviewers.'})
  }
  ...
  if(reviewerIds.includes(req.user.id))return res.status(400).json({error:'You cannot assign yourself as reviewer for a report you are preparing or submitting.'})
  if(reviewerIds.length){const valid=(await query("SELECT id FROM users WHERE id=ANY($1::uuid[]) AND active=TRUE AND role IN('Research Officer','Reviewer','Research Manager','Administrator')",[reviewerIds])).rows;if(valid.length!==reviewerIds.length)return res.status(400).json({error:'Choose only active authorized reviewers.'})}
  await transaction(async client=>{
...
/* APP2_RESEARCH_TEMPLATE_LIBRARY_V11 */
const researchTemplateLibraryV11=[{"template_key":"research-blank-report","name":"Blank Research Report","description":"Minimal unrestricted format for work that does not fit a predefined research structure.","sections":[{"key":"report_body","title":"Report Body"}]},{"template_key":"research-standard-report","name":"Standard Research Report","description":"General-purpose institutional research report suitable for most studies.","sections":[{"key":"title_page","title":"Title Page"},{"key":"document_control","title":"Document Control"},{"key":"executive_summary","title":"Executive Summary"},{"key":"introduction_and_background","title":"Introduction and Background"},{"key":"research_problem_purpose","title":"Research Problem / Purpose"},{"key":"research_questions_objectives","title":"Research Questions / Objectives"},{"key":"literature_evidence_review","title":"Literature / Evidence Review"},{"key":"methodology","title":"Methodology"},{"key":"findings_results","title":"Findings / Results"},{"key":"discussion","title":"Discussion"},{"key":"conclusions","title":"Conclusions"},{"key":"recommendations","title":"Recommendations"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-quantitative-report","name":"Quantitative Research Report","description":"For studies based primarily on numerical data and statistical analysis.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"introduction","title":"Introduction"},{"key":"problem_statement","title":"Problem Statement"},{"key":"objectives_and_hypotheses","title":"Objectives and Hypotheses"},{"key":"literature_review","title":"Literature Review"},{"key":"study_design","title":"Study Design"},{"key":"population_and_sampling","title":"Population and Sampling"},{"key":"variables_and_measures","title":"Variables and Measures"},{"key":"data_collection","title":"Data Collection"},{"key":"statistical_analysis","title":"Statistical Analysis"},{"key":"results","title":"Results"},{"key":"discussion","title":"Discussion"},{"key":"limitations","title":"Limitations"},{"key":"conclusions","title":"Conclusions"},{"key":"recommendations","title":"Recommendations"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-qualitative-report","name":"Qualitative Research Report","description":"For interviews, focus groups, observation and other qualitative inquiry.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"introduction","title":"Introduction"},{"key":"research_purpose_and_questions","title":"Research Purpose and Questions"},{"key":"context_and_literature","title":"Context and Literature"},{"key":"qualitative_approach","title":"Qualitative Approach"},{"key":"participants_and_sampling","title":"Participants and Sampling"},{"key":"data_collection","title":"Data Collection"},{"key":"data_analysis_coding","title":"Data Analysis / Coding"},{"key":"reflexivity","title":"Reflexivity"},{"key":"trustworthiness_quality","title":"Trustworthiness / Quality"},{"key":"findings_themes","title":"Findings / Themes"},{"key":"discussion","title":"Discussion"},{"key":"limitations","title":"Limitations"},{"key":"conclusions","title":"Conclusions"},{"key":"recommendations","title":"Recommendations"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-mixed-methods-report","name":"Mixed Methods Research Report","description":"For integrated quantitative and qualitative research.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"introduction","title":"Introduction"},{"key":"research_questions_and_objectives","title":"Research Questions and Objectives"},{"key":"conceptual_framework","title":"Conceptual Framework"},{"key":"mixed_methods_design","title":"Mixed Methods Design"},{"key":"quantitative_methods","title":"Quantitative Methods"},{"key":"qualitative_methods","title":"Qualitative Methods"},{"key":"integration_strategy","title":"Integration Strategy"},{"key":"quantitative_results","title":"Quantitative Results"},{"key":"qualitative_findings","title":"Qualitative Findings"},{"key":"integrated_findings","title":"Integrated Findings"},{"key":"discussion","title":"Discussion"},{"key":"limitations","title":"Limitations"},{"key":"conclusions","title":"Conclusions"},{"key":"recommendations","title":"Recommendations"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-survey-report","name":"Survey Research Report","description":"For questionnaire-based staff, citizen, customer or institutional surveys.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"background_and_objectives","title":"Background and Objectives"},{"key":"survey_population","title":"Survey Population"},{"key":"sampling_design","title":"Sampling Design"},{"key":"questionnaire_instrument","title":"Questionnaire / Instrument"},{"key":"data_collection","title":"Data Collection"},{"key":"response_rate_and_data_quality","title":"Response Rate and Data Quality"},{"key":"respondent_profile","title":"Respondent Profile"},{"key":"key_results","title":"Key Results"},{"key":"subgroup_findings","title":"Subgroup Findings"},{"key":"discussion","title":"Discussion"},{"key":"limitations","title":"Limitations"},{"key":"conclusions","title":"Conclusions"},{"key":"recommendations","title":"Recommendations"},{"key":"references","title":"References"},{"key":"questionnaire","title":"Questionnaire"},{"key":"technical_appendices","title":"Technical Appendices"}]},{"template_key":"research-case-study-report","name":"Case Study Research Report","description":"For in-depth analysis of a programme, institution, event, process or bounded case.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"case_overview","title":"Case Overview"},{"key":"research_purpose_and_questions","title":"Research Purpose and Questions"},{"key":"context","title":"Context"},{"key":"case_selection","title":"Case Selection"},{"key":"sources_of_evidence","title":"Sources of Evidence"},{"key":"analytical_approach","title":"Analytical Approach"},{"key":"case_narrative","title":"Case Narrative"},{"key":"findings","title":"Findings"},{"key":"triangulation","title":"Triangulation"},{"key":"discussion","title":"Discussion"},{"key":"lessons_learned","title":"Lessons Learned"},{"key":"limitations","title":"Limitations"},{"key":"conclusions","title":"Conclusions"},{"key":"recommendations","title":"Recommendations"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-comparative-report","name":"Comparative Research Report","description":"For comparison of institutions, jurisdictions, programmes, groups or periods.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"background","title":"Background"},{"key":"comparison_purpose_and_questions","title":"Comparison Purpose and Questions"},{"key":"cases_units_compared","title":"Cases / Units Compared"},{"key":"comparison_framework_and_criteria","title":"Comparison Framework and Criteria"},{"key":"data_sources","title":"Data Sources"},{"key":"methodology","title":"Methodology"},{"key":"profile_of_cases","title":"Profile of Cases"},{"key":"comparative_findings","title":"Comparative Findings"},{"key":"similarities_and_differences","title":"Similarities and Differences"},{"key":"benchmarking_analysis","title":"Benchmarking Analysis"},{"key":"discussion","title":"Discussion"},{"key":"limitations","title":"Limitations"},{"key":"conclusions","title":"Conclusions"},{"key":"recommendations","title":"Recommendations"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-cross-sectional-report","name":"Cross-Sectional Study Report","description":"For studies measuring variables at a defined point or period in time.","sections":[{"key":"title_page","title":"Title Page"},{"key":"abstract_executive_summary","title":"Abstract / Executive Summary"},{"key":"introduction","title":"Introduction"},{"key":"objectives","title":"Objectives"},{"key":"study_design_and_setting","title":"Study Design and Setting"},{"key":"population","title":"Population"},{"key":"sampling","title":"Sampling"},{"key":"variables_and_measurement","title":"Variables and Measurement"},{"key":"data_collection","title":"Data Collection"},{"key":"analysis","title":"Analysis"},{"key":"results","title":"Results"},{"key":"discussion","title":"Discussion"},{"key":"limitations","title":"Limitations"},{"key":"conclusions","title":"Conclusions"},{"key":"recommendations","title":"Recommendations"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-longitudinal-report","name":"Longitudinal / Cohort Study Report","description":"For repeated observation of a population, cohort or indicators over time.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"introduction","title":"Introduction"},{"key":"objectives","title":"Objectives"},{"key":"cohort_panel_design","title":"Cohort / Panel Design"},{"key":"population_and_recruitment","title":"Population and Recruitment"},{"key":"baseline_measures","title":"Baseline Measures"},{"key":"follow_up_schedule","title":"Follow-up Schedule"},{"key":"retention_attrition","title":"Retention / Attrition"},{"key":"data_collection","title":"Data Collection"},{"key":"analysis","title":"Analysis"},{"key":"longitudinal_results","title":"Longitudinal Results"},{"key":"change_over_time","title":"Change Over Time"},{"key":"discussion","title":"Discussion"},{"key":"limitations","title":"Limitations"},{"key":"conclusions","title":"Conclusions"},{"key":"recommendations","title":"Recommendations"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-evaluation-report","name":"Programme / Policy Evaluation Report","description":"For formal evaluation of programmes, policies, projects or services.","sections":[{"key":"title_page","title":"Title Page"},{"key":"evaluation_information","title":"Evaluation Information"},{"key":"executive_summary","title":"Executive Summary"},{"key":"introduction","title":"Introduction"},{"key":"intervention_policy_description","title":"Intervention / Policy Description"},{"key":"purpose_scope_and_evaluation_questions","title":"Purpose, Scope and Evaluation Questions"},{"key":"theory_of_change_results_framework","title":"Theory of Change / Results Framework"},{"key":"evaluation_criteria_and_matrix","title":"Evaluation Criteria and Matrix"},{"key":"methodology","title":"Methodology"},{"key":"ethics_and_limitations","title":"Ethics and Limitations"},{"key":"findings_by_evaluation_question","title":"Findings by Evaluation Question"},{"key":"conclusions","title":"Conclusions"},{"key":"lessons_learned","title":"Lessons Learned"},{"key":"recommendations","title":"Recommendations"},{"key":"management_response_action_matrix","title":"Management Response / Action Matrix"},{"key":"references","title":"References"},{"key":"annexes","title":"Annexes"}]},{"template_key":"research-baseline-report","name":"Baseline Study Report","description":"For pre-intervention benchmark measurement.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"background","title":"Background"},{"key":"purpose_and_baseline_objectives","title":"Purpose and Baseline Objectives"},{"key":"results_framework_indicators","title":"Results Framework / Indicators"},{"key":"methodology","title":"Methodology"},{"key":"sampling","title":"Sampling"},{"key":"data_collection","title":"Data Collection"},{"key":"data_quality","title":"Data Quality"},{"key":"baseline_characteristics","title":"Baseline Characteristics"},{"key":"indicator_baseline_results","title":"Indicator Baseline Results"},{"key":"equity_subgroup_analysis","title":"Equity / Subgroup Analysis"},{"key":"discussion","title":"Discussion"},{"key":"limitations","title":"Limitations"},{"key":"conclusions","title":"Conclusions"},{"key":"recommendations_for_implementation_monitoring","title":"Recommendations for Implementation / Monitoring"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-midline-report","name":"Midline Study Report","description":"For interim measurement and course correction.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"programme_study_background","title":"Programme / Study Background"},{"key":"midline_purpose","title":"Midline Purpose"},{"key":"methodology","title":"Methodology"},{"key":"comparability_with_baseline","title":"Comparability with Baseline"},{"key":"implementation_progress","title":"Implementation Progress"},{"key":"midline_indicator_results","title":"Midline Indicator Results"},{"key":"change_from_baseline","title":"Change from Baseline"},{"key":"emerging_outcomes","title":"Emerging Outcomes"},{"key":"implementation_challenges","title":"Implementation Challenges"},{"key":"discussion","title":"Discussion"},{"key":"limitations","title":"Limitations"},{"key":"course_correction_recommendations","title":"Course-correction Recommendations"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-endline-report","name":"Endline Study Report","description":"For final-period measurement and comparison with baseline or targets.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"background","title":"Background"},{"key":"endline_purpose_and_questions","title":"Endline Purpose and Questions"},{"key":"methodology","title":"Methodology"},{"key":"comparability_with_baseline_midline","title":"Comparability with Baseline / Midline"},{"key":"final_indicator_results","title":"Final Indicator Results"},{"key":"change_over_time","title":"Change Over Time"},{"key":"outcome_findings","title":"Outcome Findings"},{"key":"subgroup_equity_findings","title":"Subgroup / Equity Findings"},{"key":"discussion","title":"Discussion"},{"key":"limitations","title":"Limitations"},{"key":"conclusions","title":"Conclusions"},{"key":"recommendations","title":"Recommendations"},{"key":"lessons_learned","title":"Lessons Learned"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-impact-evaluation-report","name":"Impact Evaluation Report","description":"For causal or counterfactual assessment of intervention effects.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"intervention_and_theory_of_change","title":"Intervention and Theory of Change"},{"key":"evaluation_questions","title":"Evaluation Questions"},{"key":"impact_evaluation_design","title":"Impact Evaluation Design"},{"key":"counterfactual_comparison_strategy","title":"Counterfactual / Comparison Strategy"},{"key":"sampling_and_power","title":"Sampling and Power"},{"key":"outcome_measures","title":"Outcome Measures"},{"key":"data_collection","title":"Data Collection"},{"key":"identification_estimation_strategy","title":"Identification / Estimation Strategy"},{"key":"validity_checks","title":"Validity Checks"},{"key":"impact_results","title":"Impact Results"},{"key":"subgroup_effects","title":"Subgroup Effects"},{"key":"cost_value_considerations","title":"Cost / Value Considerations"},{"key":"sensitivity_robustness_analysis","title":"Sensitivity / Robustness Analysis"},{"key":"limitations","title":"Limitations"},{"key":"conclusions","title":"Conclusions"},{"key":"policy_programme_implications","title":"Policy / Programme Implications"},{"key":"recommendations","title":"Recommendations"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-needs-assessment-report","name":"Needs Assessment Report","description":"For identifying gaps between current conditions and desired services, capabilities or outcomes.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"background","title":"Background"},{"key":"assessment_purpose_and_scope","title":"Assessment Purpose and Scope"},{"key":"stakeholders_population","title":"Stakeholders / Population"},{"key":"needs_framework","title":"Needs Framework"},{"key":"methods_and_data_sources","title":"Methods and Data Sources"},{"key":"current_state","title":"Current State"},{"key":"desired_state_standards","title":"Desired State / Standards"},{"key":"gap_analysis","title":"Gap Analysis"},{"key":"priority_needs","title":"Priority Needs"},{"key":"root_causes_barriers","title":"Root Causes / Barriers"},{"key":"available_assets_resources","title":"Available Assets / Resources"},{"key":"options","title":"Options"},{"key":"prioritised_recommendations","title":"Prioritised Recommendations"},{"key":"implementation_considerations","title":"Implementation Considerations"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-situational-analysis-report","name":"Situational Analysis Report","description":"For current-state, trend, stakeholder, risk and opportunity analysis.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"purpose_and_scope","title":"Purpose and Scope"},{"key":"context","title":"Context"},{"key":"methods_and_sources","title":"Methods and Sources"},{"key":"current_situation","title":"Current Situation"},{"key":"trend_analysis","title":"Trend Analysis"},{"key":"stakeholder_analysis","title":"Stakeholder Analysis"},{"key":"institutional_policy_environment","title":"Institutional / Policy Environment"},{"key":"drivers_and_root_causes","title":"Drivers and Root Causes"},{"key":"strengths_assets","title":"Strengths / Assets"},{"key":"gaps_and_constraints","title":"Gaps and Constraints"},{"key":"risks_and_opportunities","title":"Risks and Opportunities"},{"key":"priority_issues","title":"Priority Issues"},{"key":"conclusions","title":"Conclusions"},{"key":"recommendations","title":"Recommendations"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-feasibility-report","name":"Feasibility Study Report","description":"For assessing whether a proposed intervention, system, programme or policy is viable.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"proposal_concept","title":"Proposal / Concept"},{"key":"purpose_and_scope","title":"Purpose and Scope"},{"key":"requirements","title":"Requirements"},{"key":"methodology","title":"Methodology"},{"key":"demand_need_assessment","title":"Demand / Need Assessment"},{"key":"technical_feasibility","title":"Technical Feasibility"},{"key":"operational_feasibility","title":"Operational Feasibility"},{"key":"institutional_legal_feasibility","title":"Institutional / Legal Feasibility"},{"key":"financial_economic_feasibility","title":"Financial / Economic Feasibility"},{"key":"human_resource_feasibility","title":"Human Resource Feasibility"},{"key":"risk_assessment","title":"Risk Assessment"},{"key":"options_analysis","title":"Options Analysis"},{"key":"cost_benefit_value_considerations","title":"Cost-Benefit / Value Considerations"},{"key":"implementation_requirements","title":"Implementation Requirements"},{"key":"conclusions","title":"Conclusions"},{"key":"go_no_go_recommendation","title":"Go / No-Go Recommendation"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-policy-report","name":"Policy Research Report","description":"For evidence-to-policy analysis of a public problem, options and implementation implications.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"policy_issue","title":"Policy Issue"},{"key":"background_and_context","title":"Background and Context"},{"key":"problem_definition","title":"Problem Definition"},{"key":"policy_questions","title":"Policy Questions"},{"key":"evidence_and_methods","title":"Evidence and Methods"},{"key":"stakeholder_institutional_analysis","title":"Stakeholder / Institutional Analysis"},{"key":"current_policy_legal_framework","title":"Current Policy / Legal Framework"},{"key":"findings","title":"Findings"},{"key":"policy_options","title":"Policy Options"},{"key":"options_appraisal","title":"Options Appraisal"},{"key":"preferred_option","title":"Preferred Option"},{"key":"implementation_considerations","title":"Implementation Considerations"},{"key":"risks_and_mitigation","title":"Risks and Mitigation"},{"key":"monitoring_indicators","title":"Monitoring Indicators"},{"key":"conclusions","title":"Conclusions"},{"key":"recommendations","title":"Recommendations"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-systematic-review-report","name":"Systematic Review / Evidence Synthesis Report","description":"For transparent evidence synthesis with documented search, selection and appraisal.","sections":[{"key":"title_page","title":"Title Page"},{"key":"structured_abstract","title":"Structured Abstract"},{"key":"introduction_and_rationale","title":"Introduction and Rationale"},{"key":"review_question_objectives","title":"Review Question / Objectives"},{"key":"eligibility_criteria","title":"Eligibility Criteria"},{"key":"information_sources","title":"Information Sources"},{"key":"search_strategy","title":"Search Strategy"},{"key":"selection_process","title":"Selection Process"},{"key":"data_extraction","title":"Data Extraction"},{"key":"risk_of_bias_quality_appraisal","title":"Risk of Bias / Quality Appraisal"},{"key":"synthesis_methods","title":"Synthesis Methods"},{"key":"study_selection","title":"Study Selection"},{"key":"study_characteristics","title":"Study Characteristics"},{"key":"results_of_individual_studies","title":"Results of Individual Studies"},{"key":"synthesis_results","title":"Synthesis Results"},{"key":"certainty_confidence_in_evidence","title":"Certainty / Confidence in Evidence"},{"key":"discussion","title":"Discussion"},{"key":"limitations","title":"Limitations"},{"key":"conclusions","title":"Conclusions"},{"key":"recommendations_implications","title":"Recommendations / Implications"},{"key":"references","title":"References"},{"key":"search_strategies_and_appendices","title":"Search Strategies and Appendices"}]},{"template_key":"research-scoping-review-report","name":"Scoping Review Report","description":"For broad evidence mapping, concepts and research-gap identification.","sections":[{"key":"title_page","title":"Title Page"},{"key":"abstract_executive_summary","title":"Abstract / Executive Summary"},{"key":"introduction","title":"Introduction"},{"key":"review_questions","title":"Review Questions"},{"key":"eligibility_criteria","title":"Eligibility Criteria"},{"key":"information_sources_and_search","title":"Information Sources and Search"},{"key":"selection_process","title":"Selection Process"},{"key":"data_charting","title":"Data Charting"},{"key":"synthesis_mapping_approach","title":"Synthesis / Mapping Approach"},{"key":"results","title":"Results"},{"key":"evidence_map","title":"Evidence Map"},{"key":"concepts_themes","title":"Concepts / Themes"},{"key":"research_gaps","title":"Research Gaps"},{"key":"discussion","title":"Discussion"},{"key":"limitations","title":"Limitations"},{"key":"conclusions","title":"Conclusions"},{"key":"recommendations","title":"Recommendations"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-rapid-evidence-report","name":"Rapid Evidence Assessment Report","description":"For time-bounded review using explicitly streamlined evidence methods.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"decision_policy_question","title":"Decision / Policy Question"},{"key":"scope_and_rapid_review_protocol","title":"Scope and Rapid Review Protocol"},{"key":"eligibility_criteria","title":"Eligibility Criteria"},{"key":"search_sources_and_strategy","title":"Search Sources and Strategy"},{"key":"screening_approach","title":"Screening Approach"},{"key":"quality_appraisal","title":"Quality Appraisal"},{"key":"evidence_summary","title":"Evidence Summary"},{"key":"key_findings","title":"Key Findings"},{"key":"evidence_gaps","title":"Evidence Gaps"},{"key":"strength_confidence_of_evidence","title":"Strength / Confidence of Evidence"},{"key":"limitations_of_rapid_method","title":"Limitations of Rapid Method"},{"key":"conclusions","title":"Conclusions"},{"key":"recommendations","title":"Recommendations"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-literature-review-report","name":"Literature Review Report","description":"For narrative or structured synthesis of published and grey literature.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"background","title":"Background"},{"key":"review_purpose_and_questions","title":"Review Purpose and Questions"},{"key":"scope","title":"Scope"},{"key":"search_source_identification","title":"Search / Source Identification"},{"key":"inclusion_and_exclusion_approach","title":"Inclusion and Exclusion Approach"},{"key":"literature_profile","title":"Literature Profile"},{"key":"thematic_synthesis","title":"Thematic Synthesis"},{"key":"areas_of_agreement","title":"Areas of Agreement"},{"key":"areas_of_debate","title":"Areas of Debate"},{"key":"evidence_gaps","title":"Evidence Gaps"},{"key":"discussion","title":"Discussion"},{"key":"conclusions","title":"Conclusions"},{"key":"recommendations_research_agenda","title":"Recommendations / Research Agenda"},{"key":"references","title":"References"}]},{"template_key":"research-desk-review-report","name":"Desk Review Report","description":"For analysis of existing documents, administrative data, policies and secondary sources.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"purpose_and_scope","title":"Purpose and Scope"},{"key":"review_questions","title":"Review Questions"},{"key":"documents_data_reviewed","title":"Documents / Data Reviewed"},{"key":"review_method","title":"Review Method"},{"key":"source_quality_and_limitations","title":"Source Quality and Limitations"},{"key":"findings_by_question","title":"Findings by Question"},{"key":"cross_document_analysis","title":"Cross-document Analysis"},{"key":"gaps_inconsistencies","title":"Gaps / Inconsistencies"},{"key":"conclusions","title":"Conclusions"},{"key":"recommendations","title":"Recommendations"},{"key":"source_register","title":"Source Register"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-action-research-report","name":"Action Research Report","description":"For iterative practitioner research organised around action, observation and reflection.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"context_and_problem","title":"Context and Problem"},{"key":"research_purpose","title":"Research Purpose"},{"key":"participants_stakeholders","title":"Participants / Stakeholders"},{"key":"action_research_cycle_design","title":"Action Research Cycle / Design"},{"key":"baseline_initial_understanding","title":"Baseline / Initial Understanding"},{"key":"planned_intervention_action","title":"Planned Intervention / Action"},{"key":"data_collection","title":"Data Collection"},{"key":"observations_and_results","title":"Observations and Results"},{"key":"reflection","title":"Reflection"},{"key":"adaptations_next_cycle","title":"Adaptations / Next Cycle"},{"key":"learning","title":"Learning"},{"key":"limitations","title":"Limitations"},{"key":"conclusions","title":"Conclusions"},{"key":"recommendations","title":"Recommendations"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-organisational-diagnostic-report","name":"Institutional / Organisational Diagnostic Report","description":"For organisational structure, process, capability and performance diagnostics.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"institution_directorate_profile","title":"Institution / Directorate Profile"},{"key":"purpose_and_scope","title":"Purpose and Scope"},{"key":"diagnostic_framework","title":"Diagnostic Framework"},{"key":"methodology","title":"Methodology"},{"key":"governance_and_structure","title":"Governance and Structure"},{"key":"processes_and_service_delivery","title":"Processes and Service Delivery"},{"key":"people_and_capability","title":"People and Capability"},{"key":"technology_information","title":"Technology / Information"},{"key":"performance_and_results","title":"Performance and Results"},{"key":"culture_change_readiness","title":"Culture / Change Readiness"},{"key":"stakeholder_findings","title":"Stakeholder Findings"},{"key":"strengths","title":"Strengths"},{"key":"gaps_and_root_causes","title":"Gaps and Root Causes"},{"key":"risks","title":"Risks"},{"key":"priority_interventions","title":"Priority Interventions"},{"key":"recommendations","title":"Recommendations"},{"key":"implementation_roadmap","title":"Implementation Roadmap"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-benchmarking-report","name":"Benchmarking Report","description":"For comparison of performance or practices against peers, standards or benchmarks.","sections":[{"key":"title_page","title":"Title Page"},{"key":"executive_summary","title":"Executive Summary"},{"key":"benchmarking_objective","title":"Benchmarking Objective"},{"key":"scope_and_benchmark_criteria","title":"Scope and Benchmark Criteria"},{"key":"comparator_selection","title":"Comparator Selection"},{"key":"data_sources_and_method","title":"Data Sources and Method"},{"key":"current_performance","title":"Current Performance"},{"key":"comparator_profiles","title":"Comparator Profiles"},{"key":"benchmark_results","title":"Benchmark Results"},{"key":"performance_gaps","title":"Performance Gaps"},{"key":"good_practices","title":"Good Practices"},{"key":"transferability_assessment","title":"Transferability Assessment"},{"key":"priority_opportunities","title":"Priority Opportunities"},{"key":"recommendations","title":"Recommendations"},{"key":"implementation_considerations","title":"Implementation Considerations"},{"key":"references","title":"References"},{"key":"appendices","title":"Appendices"}]},{"template_key":"research-technical-report","name":"Technical Research Report","description":"For detailed methods, specifications, analysis, validation and reproducibility.","sections":[{"key":"title_page","title":"Title Page"},{"key":"document_control","title":"Document Control"},{"key":"abstract_executive_summary","title":"Abstract / Executive Summary"},{"key":"introduction","title":"Introduction"},{"key":"objectives","title":"Objectives"},{"key":"technical_background","title":"Technical Background"},{"key":"requirements_specifications","title":"Requirements / Specifications"},{"key":"methods_procedures","title":"Methods / Procedures"},{"key":"data_materials","title":"Data / Materials"},{"key":"analysis","title":"Analysis"},{"key":"results","title":"Results"},{"key":"validation_verification","title":"Validation / Verification"},{"key":"technical_discussion","title":"Technical Discussion"},{"key":"constraints_limitations","title":"Constraints / Limitations"},{"key":"conclusions","title":"Conclusions"},{"key":"technical_recommendations","title":"Technical Recommendations"},{"key":"references","title":"References"},{"key":"technical_appendices","title":"Technical Appendices"}]},{"template_key":"research-research-brief","name":"Research Brief / Executive Summary","description":"For concise decision-oriented communication of evidence, findings and implications.","sections":[{"key":"title","title":"Title"},{"key":"research_question_issue","title":"Research Question / Issue"},{"key":"why_it_matters","title":"Why It Matters"},{"key":"approach_evidence_base","title":"Approach / Evidence Base"},{"key":"key_findings","title":"Key Findings"},{"key":"implications","title":"Implications"},{"key":"recommendations","title":"Recommendations"},{"key":"limitations_caveats","title":"Limitations / Caveats"},{"key":"references_further_reading","title":"References / Further Reading"}]}]
const ensureResearchReportTemplates=async()=>{
  for(const template of researchTemplateLibraryV11){
    await query(`INSERT INTO document_templates(template_key,name,context,description,sections,version,governance_status,active)
      VALUES($1,$2,'Research',$3,$4::jsonb,1,'Standard',TRUE)
      ON CONFLICT(template_key) DO UPDATE SET
        name=EXCLUDED.name,
        description=EXCLUDED.description,
        sections=CASE WHEN document_templates.governance_status='Retired' THEN document_templates.sections ELSE EXCLUDED.sections END,
        ...
        updated_at=NOW()`,
      [template.template_key,template.name,template.description,JSON.stringify(template.sections)])
  }
}

app.get('/api/document-templates',authenticate,async(req,res,next)=>{try{const context=z.enum(['Assignment','Research']).parse(String(req.query.context||''));if(context==='Assignment')await ensureAssignmentFinalReportTemplate();if(context==='Research')await ensureResearchReportTemplates();res.json((await query("SELECT t.*,creator.name created_by_name,approver.name approved_by_name FROM document_templates t LEFT JOIN users creator ON creator.id=t.created_by LEFT JOIN users approver ON approver.id=t.approved_by WHERE t.context=$1 AND(t.active=TRUE OR $2::boolean) ORDER BY t.name",[context,['Administrator','Research Manager'].includes(req.user.role)])).rows)}catch(error){next(error)}})
...
app.get('/api/research-document-templates/:id/download',authenticate,async(req,res,next)=>{try{
  const researchId=z.string().uuid().parse(String(req.query.researchId||''))
  if(!await canAccessResearch(req.user,researchId))return res.status(403).json({error:'You cannot download a template for this research project.'})
  await ensureResearchReportTemplates()
  const [templateResult,projectResult]=await Promise.all([
    query("SELECT * FROM document_templates WHERE id=$1 AND context='Research' AND active=TRUE AND governance_status IN('Standard','Approved')",[req.params.id]),
    query(`SELECT p.*,lead.name lead_name FROM research_projects p LEFT JOIN users lead ON lead.id=p.lead_id WHERE p.id=$1 LIMIT 1`,[researchId])
  ])
  const t=templateResult.rows[0],p=projectResult.rows[0]
  if(!t||!p)return res.status(404).json({error:'Research template or project not found.'})
  const reference=`RES-${p.id.replace(/[^a-f0-9]/gi,'').slice(0,8).toUpperCase()}`
  const html=wordDocument(t.name,{
    Organization:'Public Service Commission',
    'Research Reference':reference,
    'Research Project':p.title,
    'Research Lead':p.lead_name||'Unassigned',
    Template:t.name,
    Classification:'INTERNAL',
    Status:'BLANK TEMPLATE',
    Date:new Date().toISOString().slice(0,10),
    'Template Version':String(t.version)
  },t.sections.map(section=>({...section,content:''})))
  res.setHeader('Content-Type','application/msword; charset=utf-8')
  res.setHeader('Content-Disposition',`attachment; filename="${t.template_key}-v${t.version}.doc"`)
  res.send(html)
}catch(error){next(error)}})
...
app.get('/api/research/:id/report',authenticate,async(req,res,next)=>{
  try{
    ...
    const existingReportSectionCount=Number((await query(
      'SELECT COUNT(*)::int count FROM research_report_sections WHERE project_id=$1',
      [req.params.id]
    )).rows[0]?.count||0)

    if(existingReportSectionCount===0){
...
app.post(
  '/api/research/:id/report/apply-template',
  authenticate,
  validate(z.object({
  ...
  async(req,res,next)=>{
    try{
      const project=(await query(
        'SELECT id,title,lead_id FROM research_projects WHERE id=$1',
        [req.params.id]
      )).rows[0]
      if(!project)return res.status(404).json({error:'Research project was not found.'})

      if(!await canAccessResearch(req.user,req.params.id)){
        return res.status(403).json({error:'You cannot access this research report.'})
      }

      const isManager=['Administrator','Research Manager'].includes(req.user.role)
      const isLead=project.lead_id===req.user.id
      if(!isManager&&!isLead){
        return res.status(403).json({error:'Only the Research Lead or an authorised research manager can apply a report template.'})
      }

      const latest=(await query(
        "SELECT status FROM research_report_versions WHERE project_id=$1 ORDER BY version_number DESC LIMIT 1",
        [req.params.id]
      )).rows[0]
      if(['Submitted','Approved'].includes(latest?.status)){
        return res.status(409).json({error:'The current report version is locked. Apply a template only while the report is editable.'})
      }

      const template=researchTemplateLibraryV11.find(
        item=>item.template_key===req.validated.templateKey
      )
      if(!template)return res.status(404).json({error:'Research report template was not found.'})

      const existing=(await query(
        `SELECT id,section_key,title,content,status,owner_id,reviewer_id
      ...
      const hasAuthoredContent=existing.some(
        section=>Boolean(String(section.content||'').trim())
      )
      if(hasAuthoredContent&&!req.validated.replaceExisting){
        return res.status(409).json({
          error:'This report already contains drafted content. Confirm template replacement before changing the report outline.',
          code:'REPORT_TEMPLATE_REPLACE_CONFIRMATION_REQUIRED'
        })
      }

      const sections=Array.isArray(template.sections)?template.sections:[]
      if(!sections.length){
        return res.status(400).json({error:'The selected template has no report sections.'})
      }

      const result=await transaction(async client=>{
        const lockedProject=(await client.query(
          'SELECT id,title,lead_id FROM research_projects WHERE id=$1 FOR UPDATE',
          [req.params.id]
        )).rows[0]
        if(!lockedProject)throw Object.assign(new Error('Research project was not found.'),{statusCode:404})
        ...
        const existingByKey=new Map(
          currentSections.map(section=>[section.section_key,section])
        )
        const keys=sections.map(section=>section.key)

        for(let index=0;index<sections.length;index+=1){
          const section=sections[index]
          const current=existingByKey.get(section.key)
          await client.query(
            `INSERT INTO research_report_sections(
               project_id,section_key,title,content,section_order,status,
               owner_id,reviewer_id,updated_by,updated_at
             )
             ...
             ON CONFLICT(project_id,section_key) DO UPDATE SET
               ...
               content=CASE
                 WHEN research_report_sections.content<>'' THEN research_report_sections.content
                 ELSE EXCLUDED.content
               END,
               status=CASE
                 WHEN research_report_sections.content<>'' THEN research_report_sections.status
            ...
            [
              ...
              current?.owner_id||lockedProject.lead_id,
              current?.reviewer_id||null,
              req.user.id
            ]
          )
        }
        ...
        await client.query(
          `DELETE FROM research_report_sections
           WHERE project_id=$1
             AND NOT(section_key=ANY($2::text[]))`,
          [lockedProject.id,keys]
        )

        await client.query(
          'INSERT INTO research_activity(project_id,user_id,action,details)VALUES($1,$2,$3,$4)',
          [
            lockedProject.id,
            req.user.id,
            ...
            JSON.stringify({
              ...
              replacedExisting:hasAuthoredContent
            })
          ]
        )
        ...
        await audit(
          client,
          req.user.id,
          'RESEARCH_REPORT_TEMPLATE_APPLIED',
          'research_project',
          lockedProject.id,
          {
            templateKey:template.template_key,
        ...
        const refreshed=await client.query(
          `SELECT
             rrs.id,rrs.section_key,rrs.title,rrs.content,rrs.section_order,
             rrs.status,rrs.owner_id,rrs.reviewer_id,rrs.updated_at,
             u.name updated_by_name,
             owner.name owner_name,
             reviewer.name reviewer_name
           FROM research_report_sections rrs
...
app.post('/api/research/:id/report/submit',authenticate,validate(z.object({title:z.string().min(3).max(300),reviewerId:z.string().uuid()})),async(req,res,next)=>{try{
  ...
  const canSubmit=['Administrator','Research Manager'].includes(req.user.role)||project.lead_id===req.user.id||isReportBuilder
  if(!canSubmit)return res.status(403).json({error:'Only the Research Lead, an active report builder, or an authorised Research Manager can submit the report for review.'})
...
app.post('/api/research/:id/report/generate',authenticate,validate(z.object({
  title:z.string().min(3).max(300),
  classification:z.enum(['Official','Internal','Confidential','Public']).default('Official'),
  templateId:z.string().uuid().nullable().default(null),
  mode:z.enum(['Draft','Final']).default('Final'),
  reviewerId:z.string().uuid().nullable().default(null),
  knowledgeIds:z.array(z.string().uuid()).max(100).default([])
})),async(req,res,next)=>{let storedFinalPath;try{
  const project=(await query('SELECT * FROM research_projects WHERE id=$1',[req.params.id])).rows[0]
  if(!project)return res.status(404).json({error:'Research project not found.'})
  ...
  if(!draftMode){
    const existingFinal=(await query(`SELECT
        gd.id generated_document_id,
        gd.reference,
        gd.title,
        k.id repository_document_id,
        k.title repository_document_title
      FROM knowledge_items k
      JOIN repository_entity_links research_link
        ON research_link.knowledge_id=k.id
       AND research_link.entity_type='research'
       AND research_link.entity_id=$1
      LEFT JOIN repository_entity_links report_link
        ON report_link.knowledge_id=k.id
       AND report_link.entity_type='report'
      LEFT JOIN generated_documents gd
        ON gd.id=report_link.entity_id
      WHERE k.document_type='Research Final Report'
        AND k.status='Published'
        AND COALESCE(k.is_archived,FALSE)=FALSE
      ORDER BY k.created_at DESC
      LIMIT 1`,[project.id])).rows[0]||null

    if(existingFinal&&project.status==='Completed'){
      return res.json({
        id:existingFinal.generated_document_id,
        reference:existingFinal.reference||'FINAL',
        title:existingFinal.title||existingFinal.repository_document_title,
        repository_document_id:existingFinal.repository_document_id,
        repository_document_title:existingFinal.repository_document_title,
  ...
  if(draftMode){
    const canDraft=isManager||project.lead_id===req.user.id
    if(!canDraft)return res.status(403).json({error:'Only the Research Lead or a research manager can generate a draft report.'})
  }

  const sections=(await query(
    'SELECT * FROM research_report_sections WHERE project_id=$1 ORDER BY section_order',
    [project.id]
  )).rows
  if(!sections.length)return res.status(409).json({error:'The research report has no sections to generate.'})
  if(draftMode&&!sections.some(section=>String(section.content||'').trim())){
    return res.status(409).json({error:'Write and save at least one report section before generating a draft.'})
  }
  if(!draftMode&&sections.some(section=>String(section.content||'').trim()&&section.status!=='Approved')){
    return res.status(409).json({error:'Approve every authored research report section before generating the final report. Blank template sections do not block finalisation.'})
  }

  const approvedVersion=!draftMode?(await query(
    "SELECT * FROM research_report_versions WHERE project_id=$1 AND status='Approved' ORDER BY version_number DESC LIMIT 1",
    [project.id]
  )).rows[0]:null

  if(!draftMode&&!approvedVersion){
    return res.status(409).json({error:'The complete research report must be submitted and approved as a locked version before final generation.'})
  }
  if(
    !draftMode &&
    !isManager &&
    approvedVersion.reviewer_id!==req.user.id &&
    approvedVersion.reviewed_by!==req.user.id
  ){
    return res.status(403).json({
      error:'Only the appointed reviewer for this approved version, the reviewer who recorded the approval decision, or an authorised research manager can generate its final report.'
    })
  }
  if(!draftMode&&!await researchHasControlledEvidence(project.id)){
    return res.status(409).json({error:'Add at least one controlled research source or link a published Repository document before generating the final report.'})
  }

  if(draftMode&&!req.validated.reviewerId){
    return res.status(400).json({error:'Choose an authorised reviewer before sending the draft report.'})
  }
  if(req.validated.reviewerId){
    const reviewer=(await query(
      "SELECT id,name FROM users WHERE id=$1 AND active=TRUE AND role IN('Reviewer','Research Manager','Administrator')",
      [req.validated.reviewerId]
    )).rows[0]
    if(!reviewer)return res.status(400).json({error:'Choose an active authorised reviewer or manager.'})
    if(draftMode&&(await query(
      'SELECT 1 FROM research_reviewers WHERE project_id=$1 AND reviewer_id=$2 AND active=TRUE',
      [project.id,req.validated.reviewerId]
    )).rowCount===0){
      return res.status(400).json({error:'Choose one of the formal reviewers assigned to this research project.'})
    }
  }

  if(req.validated.knowledgeIds.length){
    const available=(await query(
      "SELECT COUNT(*)::int total FROM knowledge_items WHERE id=ANY($1::uuid[]) AND(status='Published' OR $2='Draft')",
      [req.validated.knowledgeIds,req.validated.mode]
    )).rows[0].total
    if(available!==new Set(req.validated.knowledgeIds).size){
      return res.status(400).json({error:'Final reports may use only published Repository documents.'})
    }
  }

  // Prepare an immutable repository file for the approved final report.
  let finalFile=null
  let reference=`RSH-DOC-${new Date().getFullYear()}-${crypto.randomBytes(3).toString('hex').toUpperCase()}`
  if(!draftMode){
    if(
      !isManager &&
      approvedVersion.reviewer_id!==req.user.id &&
      ...
      return res.status(403).json({error:'Your user is neither the appointed reviewer nor the recorded approver for this approved version.'})
    }
    const approvalDate=new Date(approvedVersion.reviewed_at||Date.now())
    const approvalTime=new Intl.DateTimeFormat('en-KE',{
      dateStyle:'medium',
      timeStyle:'short',
      timeZone:'Africa/Nairobi'
    }).format(approvalDate)
    const repositorySections=sections.map(section=>({
      title:section.title,
      content:plainTaskReportText(section.content)
    }))
    repositorySections.push({
      title:'Review and Approval',
      content:`Status: FINAL — APPROVED\nReviewed and approved by: ${req.user.name||'Authorised reviewer'}\nApproval date: ${approvalTime}\nSubmitted report version: ${approvedVersion.version_number}`
    })
    const finalDocx=await buildDocx(req.validated.title,{
      Reference:reference,
      Research:project.title,
      'Research Lead':project.lead_id||'',
      'Reviewed and approved by':req.user.name||'Authorised reviewer',
      Status:'FINAL — APPROVED',
      Version:Number(approvedVersion.version_number||1),
      'Approval date':approvalTime
    },repositorySections)
    const safeBase=String(project.title||'research-report')
      .replace(/[^a-z0-9]+/gi,'-')
      .replace(/^-+|-+$/g,'')
      .slice(0,90)||'research-report'
    const originalName=`${safeBase}-final-v${Number(approvedVersion.version_number||1)}.docx`
    const stored=await storeDocument(finalDocx,'.docx')
    storedFinalPath=stored.absolutePath
    finalFile={
      buffer:finalDocx,
      originalName,
      stored,
      sha256:crypto.createHash('sha256').update(finalDocx).digest('hex'),
      mimeType:'application/vnd.openxmlformats-officedocument.wordprocessingml.document'
    }
  }
  ...
  const generated=await transaction(async client=>{
    ...
    if(!lockedProject)throw Object.assign(new Error('Research project not found.'),{statusCode:404})

    if(!draftMode){
      const already=(await client.query(`SELECT k.id,k.title
        FROM knowledge_items k
        JOIN repository_entity_links rel ON rel.knowledge_id=k.id
        WHERE rel.entity_type='research'
          AND rel.entity_id=$1
          AND k.document_type='Research Final Report'
          AND k.status='Published'
          AND COALESCE(k.is_archived,FALSE)=FALSE
        ORDER BY k.created_at DESC LIMIT 1`,[project.id])).rows[0]
      if(already){
        throw Object.assign(
          new Error('The approved final research report has already been published. Refresh Research or Documents to view the completed record.'),
          {statusCode:409}
        )
      }
    }

    const template=req.validated.templateId
      ?(await client.query(
        "SELECT * FROM document_templates WHERE id=$1 AND context='Research' AND active=TRUE AND governance_status IN('Standard','Approved')",
        [req.validated.templateId]
      )).rows[0]
      :(await client.query(
        "SELECT * FROM document_templates WHERE context='Research' AND active=TRUE AND governance_status IN('Standard','Approved') ORDER BY CASE governance_status WHEN 'Approved' THEN 0 ELSE 1 END,name LIMIT 1"
      )).rows[0]

    if(!template)throw Object.assign(new Error('No active Standard or Approved Research template is available.'),{statusCode:409})

    const document=(await client.query(
      `INSERT INTO generated_documents(
        template_id,template_version,context,context_id,title,reference,classification,status,
        created_by,last_updated_by,reviewer_id,submitted_by,submitted_at
      )VALUES($1,$2,'Research',$3,$4,$5,$6,$7,$8,$8,$9,$10,$11)RETURNING *`,
      [
        template.id,
        template.version,
        project.id,
        ...
        draftMode?'Submitted':'Final',
        req.user.id,
        draftMode?req.validated.reviewerId:req.user.id,
        draftMode?req.user.id:null,
        draftMode?new Date():null
      ]
    )).rows[0]

    for(const section of sections){
      const completion=section.status==='Approved'?100:String(section.content||'').trim()?60:0
      await client.query(
        `INSERT INTO generated_document_sections(
          document_id,section_key,title,section_order,content,completion,section_status,owner_id,updated_by
        )VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
        [
          document.id,section.section_key,section.title,section.section_order,section.content,completion,
          section.status==='Approved'?'Complete':String(section.content||'').trim()?'In Progress':'Not Started',
          section.owner_id,req.user.id
        ]
      )
    }

    for(const knowledgeId of [...new Set(req.validated.knowledgeIds)]){
      await client.query(
        "INSERT INTO repository_entity_links(knowledge_id,entity_type,entity_id,linked_by)VALUES($1,'research',$2,$3)ON CONFLICT DO NOTHING",
        [knowledgeId,project.id,req.user.id]
      )
      const item=(await client.query(
        'SELECT title,author,document_date FROM knowledge_items WHERE id=$1',
        [knowledgeId]
      )).rows[0]
      await client.query(
        "INSERT INTO generated_document_references(document_id,source_type,title,author,publication_year,publisher,url,identifier,citation_style,created_by)VALUES($1,'Repository Document',$2,$3,$4,'Public Service Commission','','','APA',$5)",
        [document.id,item.title,item.author||'',item.document_date?new Date(item.document_date).getFullYear():null,req.user.id]
      )
    }
    ...
    if(draftMode){
      await client.query(
        "INSERT INTO generated_document_reviews(document_id,version_number,reviewer_id,decision,comments)VALUES($1,$2,$3,'Submitted','Draft research report submitted for review.')",
        [document.id,document.version,req.validated.reviewerId]
      )
      await client.query(
        "INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Draft research report ready for review',$2,'generated_document',$3)",
        [req.validated.reviewerId,`Review “${document.title}” for research project “${project.title}”.`,document.id]
      )
    }else{
      const category=(await client.query(
        "SELECT id FROM document_categories WHERE name='Research Reports' LIMIT 1"
      )).rows[0]||null
      const tags=['research','final-report','approved']
      const knowledgeClassification={
        Public:'PUBLIC',
        Confidential:'CONFIDENTIAL',
        Internal:'INTERNAL',
        Official:'INTERNAL'
      }[req.validated.classification]||'INTERNAL'
      const description=`Final approved research report for “${project.title}”. Approved and generated by ${req.user.name||'the assigned reviewer'}.`

      const item=(await client.query(`INSERT INTO knowledge_items(
        title,description,category,category_id,tags,author,document_date,status,created_by,approved_by,approved_at,current_version,
        source_type,source_url,directorate,document_type,subject,classification,felix_enabled,reviewed_by,reviewed_at
      )VALUES(
        $1,$2,'Research Reports',$3,$4,$5,CURRENT_DATE,'Published',$6,$6,NOW(),1,
        'Research','',$7,'Research Final Report',$8,$9,TRUE,$6,NOW()
      )RETURNING *`,[
        req.validated.title,
        description,
        category?.id||null,
      ...
      const version=(await client.query(`INSERT INTO knowledge_versions(
        knowledge_id,version_number,uploader_id,original_name,stored_name,storage_path,mime_type,size_bytes,sha256_hash,
        is_current,approved_by,approved_at,notes
      )VALUES($1,1,$2,$3,$4,$5,$6,$7,$8,TRUE,$2,NOW(),
        'Automatically generated from reviewer-approved research report'
      )RETURNING *`,[
        item.id,
        req.user.id,
        finalFile.originalName,
        finalFile.stored.storedName,
        finalFile.stored.storagePath,
        finalFile.mimeType,
        finalFile.buffer.length,
        finalFile.sha256
      ])).rows[0]

      for(const tag of tags){
        const tagRow=(await client.query(
          'INSERT INTO document_tags(name,normalized_name,created_by)VALUES($1::varchar,lower($1::text)::varchar,$2::uuid)ON CONFLICT(normalized_name)DO UPDATE SET name=EXCLUDED.name RETURNING id',
          [tag,req.user.id]
        )).rows[0]
        await client.query(
          'INSERT INTO document_tag_links(knowledge_id,tag_id)VALUES($1::uuid,$2::uuid)ON CONFLICT DO NOTHING',
          [item.id,tagRow.id]
        )
      }
      ...
      await client.query(
        "INSERT INTO repository_entity_links(knowledge_id,entity_type,entity_id,linked_by)VALUES($1,'research',$2,$3)ON CONFLICT DO NOTHING",
        [item.id,project.id,req.user.id]
      )
      await client.query(
        "INSERT INTO repository_entity_links(knowledge_id,entity_type,entity_id,linked_by)VALUES($1,'report',$2,$3)ON CONFLICT DO NOTHING",
        [item.id,document.id,req.user.id]
      )

      await client.query(
        "UPDATE research_projects SET status='Completed',updated_at=NOW() WHERE id=$1",
        [project.id]
      )
      await client.query(
        'INSERT INTO research_activity(project_id,user_id,action,details)VALUES($1,$2,$3,$4)',
        [
          project.id,
          req.user.id,
          'FINAL_REPORT_PUBLISHED_AND_RESEARCH_COMPLETED',
          JSON.stringify({
            documentId:document.id,
            repositoryDocumentId:item.id,
            repositoryVersionId:version.id,
            reference,
            approvedVersion:Number(approvedVersion.version_number||1)
          })
        ]
      )
      await audit(
        client,
        req.user.id,
        'RESEARCH_FINAL_REPORT_PUBLISHED',
        'knowledge',
        item.id,
        {
      ...
      await audit(
        client,
        req.user.id,
        'RESEARCH_STATUS_UPDATED',
        'research_project',
        project.id,
        {from:lockedProject.status,to:'Completed',reason:'Final approved report published'}
      )
      await enqueueFelixDocumentIndex(client,item.id,1,req.user.id)

      if(project.lead_id){
        await client.query(
          "INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Research completed',$2,'research',$3)",
          [
            project.lead_id,
            `The approved final report for “${project.title}” was generated by ${req.user.name||'the assigned reviewer'}, published to Documents, and the research project was marked Completed.`,
            project.id
          ]
        )
      }

      repository={id:item.id,title:item.title}
    }
    ...
    if(draftMode){
      await client.query(
        'INSERT INTO research_activity(project_id,user_id,action,details)VALUES($1,$2,$3,$4)',
        [
          project.id,req.user.id,'DRAFT_REPORT_SUBMITTED',
          JSON.stringify({documentId:document.id,reference,title:document.title,sectionCount:sections.length,reviewerId:req.validated.reviewerId})
        ]
      )
    }
    ...
    await audit(
      client,
      req.user.id,
      'RESEARCH_CONTROLLED_REPORT_GENERATED',
      'generated_document',
      document.id,
      {projectId:project.id,reference,sectionCount:sections.length,templateId:template.id,mode:req.validated.mode}
    )
    ...
    return {
      ...
      repository_document_id:repository?.id||null,
      repository_document_title:repository?.title||null,
      project_status:draftMode?lockedProject.status:'Completed'
    }
  })
  ...
  if(storedFinalPath)await fs.unlink(storedFinalPath).catch(()=>{})
  ...
  try{
    ...
    const reviews=await query(`
      SELECT * FROM (
        SELECT
          ...
          'Generated Document' review_kind,
          'Review' type
        FROM generated_documents gd
        ...
        WHERE t.reviewer_id=$1
          AND t.archived_at IS NULL
          AND t.contribution_status='Integrated'
      ...
      ORDER BY
        CASE WHEN type='Review' THEN 0 ELSE 1 END,
        due_date NULLS LAST,
        updated_at DESC
      LIMIT 40
    `,[req.user.id])

    // Completed-work archive is supplemental and must never break the core management dashboard.
    const completedWork=management
      ? await Promise.allSettled([
          query(`SELECT
              a.id,
              a.title,
              a.status,
              a.updated_at completed_at,
              lead.name lead_name
            FROM assignments a
            LEFT JOIN LATERAL(
              SELECT u.name
              ...
              WHERE am.assignment_id=a.id
                AND am.member_role='Lead'
              ORDER BY am.created_at NULLS LAST,u.name
              LIMIT 1
            ) lead ON TRUE
    ...
    const normalized=sourceRows.map(item=>{
      ...
      return {
        ...
        nextAction:item.type==='Finalization'?'Generate final report':item.type==='Review'?'Review':item.type==='Task'?'Continue task':item.type==='Research Project'||item.type==='Research Milestone'?'Open research':'Open assignment',
        destination:(item.type==='Review'||item.type==='Finalization')?reviewDestination:item.type==='Research Project'||item.type==='Research Milestone'?'Research Repository':'Assignments',
        contextId:(item.type==='Review'||item.type==='Finalization')?(item.context_id||item.id):(item.assignment_id||item.project_id||item.id),
        contextTitle:item.context_title||null,
        reviewKind:item.review_kind||null,
    ...
    const reviewWork=unique
      .filter(item=>item.type==='Review'||item.type==='Finalization')
      .sort((a,b)=>{
    ...
    const overdueSource=researcher?tasks.rows:reviewerOnly?reviews.rows.filter(item=>item.type==='Review'):assignments.rows
    ...
    res.json({
      ...
      attention:{assignments:assignmentAttention,reviews:reviews.rows.filter(item=>item.type==='Review').length,overdue,notifications:notifications.rows.length,almostDue},
      ...
      management:management?{
        ...
        completed:{
          assignments:completedWork[0].rows,
          tasks:completedWork[1].rows,
          research:completedWork[2].rows
        }
      }:null
    })
  }catch(error){next(error)}
...
FROM knowledge_items k JOIN users u ON u.id=k.created_by LEFT JOIN users reviewer ON reviewer.id=COALESCE(k.reviewed_by,k.reviewer_id) LEFT JOIN users approver ON approver.id=k.approved_by LEFT JOIN document_locks l ON l.knowledge_id=k.id AND l.expires_at>NOW() LEFT JOIN users lu ON lu.id=l.locked_by LEFT JOIN LATERAL(SELECT status,attempts,last_error,updated_at FROM felix_document_index_jobs WHERE knowledge_id=k.id AND version_number=k.current_version ORDER BY id DESC LIMIT 1)fj ON TRUE WHERE k.status='Published' OR k.created_by=$1 OR $2 ORDER BY k.updated_at DESC`,[req.user.id,['Administrator','Research Manager','Reviewer'].includes(req.user.role)])).rows)}catch(error){next(error)}})

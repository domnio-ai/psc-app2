...
export default function CalendarView({ items }: { items: CalendarItem[] }) {
  ...
  return <div className="calendar-workspace">
    ...
    <aside className="calendar-agenda"><header><small>SELECTED DATE</small><h3>{new Date(`${selected}T12:00:00`).toLocaleDateString('en-KE', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</h3></header>{selectedItems.map(item => <article key={`${item.type}-${item.id}`}><b>{item.type === 'assignment' ? 'Assignment deadline' : 'Notice Board event'}</b><h4>{item.title}</h4><time>{new Date(item.start_at).toLocaleString('en-KE')}{item.end_at ? ` – ${new Date(item.end_at).toLocaleString('en-KE')}` : ''}</time><em>{item.status}</em></article>)}{!selectedItems.length && <div className="calendar-empty"><strong>No events on this date</strong><p>Select a highlighted date, or add due dates to assignments and event dates to approved notices.</p></div>}</aside>
  </div>
}

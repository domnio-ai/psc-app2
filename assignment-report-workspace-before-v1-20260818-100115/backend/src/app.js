...
const assignmentDocumentRole=async(user,document)=>{
  if(!document||document.context!=='Assignment')return null
  if(['Administrator','Research Manager'].includes(user.role))return 'Manager'
  if(document.reviewer_id===user.id)return 'Reviewer'
  ...
  if(membership?.member_role==='Lead')return 'Lead'
  if(membership)return 'Contributor'
  return document.created_by===user.id?'Lead':null
}
...
app.get('/api/generated-documents/:id',authenticate,async(req,res,next)=>{try{
  ...
  res.json({...document,current_user_role:role,sections})
}catch(error){next(error)}})
...
app.patch('/api/generated-documents/:id/sections/:sectionId',authenticate,validate(z.object({content:z.string().max(500000),completion:z.number().int().min(0).max(100),sectionStatus:z.enum(['Not Started','In Progress','Ready','Needs Changes','Complete']).optional()})),async(req,res,next)=>{try{
  ...
  if(document.context==='Assignment'&&!['Lead','Manager'].includes(role)&&section.owner_id!==req.user.id)return res.status(403).json({error:'Only the section owner, Assignment Lead or manager can edit this section.'})
  const updated=(await transaction(async client=>{
    const row=(await client.query(`UPDATE generated_document_sections SET content=$1,completion=$2,section_status=COALESCE($3,section_status),updated_by=$4::uuid,updated_at=NOW() WHERE id=$5::uuid RETURNING *`,[req.validated.content,req.validated.completion,req.validated.sectionStatus||null,req.user.id,section.id])).rows[0]

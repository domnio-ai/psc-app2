...
const assignmentTaskContributionReviewSchema=z.object({
  decision:z.enum(['Changes Requested','Accepted']),
...
app.post('/api/assignments/:id/tasks/:taskId/contribution-review',authenticate,validate(assignmentTaskContributionReviewSchema),async(req,res,next)=>{try{
  ...
  if(!['Ready for Integration','Integrated'].includes(current.contribution_status))return res.status(409).json({error:'This task report is not currently submitted for review.'})
  ...
  const updated=await transaction(async client=>{
    const row=(await client.query(`UPDATE assignment_tasks SET
      ...
      status=$2,
      progress=CASE WHEN $1='Accepted' THEN 100 ELSE progress END,
      contribution_integrated_at=CASE WHEN $1='Accepted' THEN NOW() ELSE contribution_integrated_at END,
      contribution_integrated_by=CASE WHEN $1='Accepted' THEN $3 ELSE contribution_integrated_by END,
      updated_at=NOW()
      WHERE id=$4 AND assignment_id=$5 RETURNING *`,[
        decision==='Accepted'?'Accepted':'Draft',
        decision==='Accepted'?'Completed':'In Progress',
        req.user.id,
        req.params.taskId,
        req.params.id
      ])).rows[0]
    const action=decision==='Accepted'?'TASK_REPORT_ACCEPTED':'TASK_REPORT_CHANGES_REQUESTED'
    await client.query('INSERT INTO assignment_history(assignment_id,user_id,action,details)VALUES($1,$2,$3,$4)',[
      req.params.id,req.user.id,action,JSON.stringify({taskId:updated.id,title:updated.title,decision,comments,reviewerId:req.user.id})
    ])
    if(updated.owner_id){
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,$2,$3,'assignment_task',$4)",[
        updated.owner_id,
        decision==='Accepted'?'Task report accepted':'Changes requested on task report',
        decision==='Accepted'?`Your task report for “${updated.title}” was accepted.`:`Changes were requested on “${updated.title}”. ${comments}`,
        updated.id
      ])
    }
    await audit(client,req.user.id,decision==='Accepted'?'ASSIGNMENT_TASK_REPORT_ACCEPTED':'ASSIGNMENT_TASK_REPORT_CHANGES_REQUESTED','assignment_task',updated.id,{assignmentId:req.params.id,comments})

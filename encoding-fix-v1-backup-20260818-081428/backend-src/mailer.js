...
export async function sendMail({to,subject,text,attachments=[]}){
  ...
  const info=await transport.sendMail({from:config.smtp.from,to,subject,text,attachments,html:`<div style="font-family:Arial,sans-serif;max-width:640px"><h2 style="color:#8a6a00">PSC App2</h2><p>${text.replace(/[&<>]/g,value=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[value]))}</p><hr><small>Public Service Commission Research Department</small></div>`})

...
app.patch('/api/assignments/:id/tasks/:taskId/contribution',authenticate,validate(assignmentTaskContributionSchema),async(req,res,next)=>{try{
  ...
  const taskReviewer=manager||teamLead||exactReviewer
  if(!taskReviewer&&current.owner_id!==req.user.id)return res.status(403).json({error:'Only the task owner, assigned reviewer, team lead or a manager can update this contribution.'})
  if(!taskReviewer&&current.owner_id===req.user.id&&current.status==='Blocked')return res.status(409).json({error:'This task report was rejected and is locked. A manager must reopen it for revision before you can edit it.'})
  if(!taskReviewer&&current.owner_id===req.user.id&&current.contribution_status!=='Draft')return res.status(409).json({error:'This submitted task report is locked while it is under review. Wait for the reviewer decision or requested changes.'})
  const v=req.validated
  if(!taskReviewer&&['Integrated','Accepted'].includes(v.contributionStatus))return res.status(403).json({error:'Only the assigned reviewer, assignment team lead or a manager can review and approve a task.'})
  const assignment=(await query('SELECT title FROM assignments WHERE id=$1',[req.params.id])).rows[0]
  ...
  const row=await transaction(async client=>{
    ...
    if(v.contributionStatus==='Ready for Integration'){
      const body=`${req.user.name||current.owner_name||'A team member'} submitted the task report for “${updated.title}” in “${assignment?.title||'the assignment'}”.`
      if(current.reviewer_id){
        await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Task report ready for review',$2,'assignment_task',$3)",[current.reviewer_id,body,updated.id])
      }else{
        await client.query(`INSERT INTO notifications(user_id,title,body,entity_type,entity_id)
          SELECT DISTINCT u.id,'Task report ready for review',$1,'assignment_task',$2
          FROM users u
          LEFT JOIN assignment_members am ON am.user_id=u.id AND am.assignment_id=$3
          WHERE u.active=TRUE AND (u.role IN('Administrator','Research Manager') OR am.member_role='Lead')`,[body,updated.id,req.params.id])
      }
    }
    return updated
  })
...
app.post('/api/assignments/:id/tasks/:taskId/contribution-review',authenticate,validate(assignmentTaskContributionReviewSchema),async(req,res,next)=>{try{
  ...
  const teamLead=(await query("SELECT 1 FROM assignment_members WHERE assignment_id=$1 AND user_id=$2 AND member_role='Lead'",[req.params.id,req.user.id])).rowCount>0
  const exactReviewer=current.reviewer_id===req.user.id
  if(!exactReviewer&&!teamLead&&!manager)return res.status(403).json({error:'Only the assigned reviewer, assignment Team Lead or a manager can decide this task report.'})
  ...
  if(['Changes Requested','Rejected'].includes(decision)&&!comments)return res.status(400).json({error:decision==='Rejected'?'A rejection reason is required.':'Add clear review comments before requesting changes.'})
  const updated=await transaction(async client=>{
    const nextContributionStatus=decision==='Approved'?'Integrated':'Draft'
    const nextTaskStatus=decision==='Rejected'?'Blocked':'In Progress'
    const row=(await client.query(`UPDATE assignment_tasks SET
      contribution_status=$1,
      status=$2,
      contribution_integrated_at=CASE WHEN $1='Integrated' THEN NOW() ELSE contribution_integrated_at END,
      contribution_integrated_by=CASE WHEN $1='Integrated' THEN $3 ELSE contribution_integrated_by END,
      updated_at=NOW()
      WHERE id=$4 AND assignment_id=$5 RETURNING *`,[
    ...
    await client.query('INSERT INTO assignment_history(assignment_id,user_id,action,details)VALUES($1,$2,$3,$4)',[
      req.params.id,req.user.id,action,JSON.stringify({taskId:row.id,title:row.title,decision,comments,reviewerId:req.user.id,reportVersion:Number(row.contribution_report_version||0)})
    ])
    if(row.owner_id){
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,$2,$3,'assignment_task',$4)",[
        row.owner_id,
        decision==='Approved'?'Task report approved':decision==='Rejected'?'Task report rejected':'Changes requested on task report',
        decision==='Approved'
    ...
    if(decision==='Rejected'){
      await client.query(`INSERT INTO notifications(user_id,title,body,entity_type,entity_id)
        SELECT DISTINCT u.id,'Rejected task requires management action',$1,'assignment_task',$2
        FROM users u
        LEFT JOIN assignment_members am ON am.user_id=u.id AND am.assignment_id=$3
        WHERE u.active=TRUE AND u.id<>$4 AND (u.role IN('Administrator','Research Manager') OR am.member_role='Lead')`,[
          `“${row.title}” was rejected by ${req.user.name||'the assigned reviewer'}. Review the reason and reopen for revision or reassign the task.`,row.id,req.params.id,req.user.id
      ])
    }
    if(decision==='Approved'){
      const finalizers=[current.reviewer_id].filter(Boolean)
      for(const userId of finalizers){
        await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Final task report ready to generate',$2,'assignment_task',$3)",[
          userId,`“${row.title}” is approved. Generate the final report to complete the task.`,row.id
        ])
      }
    }
    await audit(client,req.user.id,decision==='Approved'?'ASSIGNMENT_TASK_REPORT_APPROVED':decision==='Rejected'?'ASSIGNMENT_TASK_REPORT_REJECTED':'ASSIGNMENT_TASK_REPORT_CHANGES_REQUESTED','assignment_task',row.id,{assignmentId:req.params.id,comments,reportVersion:Number(row.contribution_report_version||0)})
    return row
  })
  const joined=(await query(`SELECT t.*,u.name owner_name,reviewer.name reviewer_name,gd.title target_document_title,gds.title target_section_title,assignment_section.title assignment_section_title
    ...
    LEFT JOIN generated_document_sections gds ON gds.id=t.target_section_id
    LEFT JOIN assignment_sections assignment_section ON assignment_section.id=t.assignment_section_id
    WHERE t.id=$1`,[updated.id])).rows[0]
  res.json(joined)
}catch(error){next(error)}})
...
app.post('/api/assignments/:id/tasks/:taskId/contribution-finalize',authenticate,async(req,res,next)=>{let storedFinalPath;try{
  ...
  const repositoryTitle=String(current.contribution_title||'').trim()||`${current.title} — Final Task Report`
  const repositorySections=[
    {title:'Task Purpose',content:current.task_purpose||current.description||''},
    {title:'Instructions',content:current.specific_instructions||''},
    {title:'Expected Contribution',content:current.expected_contribution||''},
    {title:'Work Completed',content:current.contribution_summary||current.notes||''},
    {title:'Evidence Reviewed',content:current.evidence_reviewed||''},
    {title:'Key Findings',content:current.contribution_findings||''},
    {title:'Challenges and Limitations',content:current.contribution_challenges||''},
    {title:'Recommendations',content:current.contribution_recommendations||''},
    {title:'Next Actions',content:current.contribution_next_actions||''},
  ...
  const result=await transaction(async client=>{
    ...
    for(const tag of tags){
      const tagRow=(await client.query('INSERT INTO document_tags(name,normalized_name,created_by)VALUES($1,lower($1),$2)ON CONFLICT(normalized_name)DO UPDATE SET name=EXCLUDED.name RETURNING id',[tag,req.user.id])).rows[0]
      await client.query('INSERT INTO document_tag_links(knowledge_id,tag_id)VALUES($1,$2)ON CONFLICT DO NOTHING',[item.id,tagRow.id])
    }
    await client.query("INSERT INTO repository_entity_links(knowledge_id,entity_type,entity_id,linked_by)VALUES($1,'task',$2,$3)ON CONFLICT DO NOTHING",[item.id,current.id,req.user.id])
    await client.query("INSERT INTO repository_entity_links(knowledge_id,entity_type,entity_id,linked_by)VALUES($1,'assignment',$2,$3)ON CONFLICT DO NOTHING",[item.id,req.params.id,req.user.id])
    await client.query('INSERT INTO knowledge_assignment_links(knowledge_id,assignment_id,linked_by)VALUES($1,$2,$3)ON CONFLICT DO NOTHING',[item.id,req.params.id,req.user.id])
...
}catch(error){if(storedFinalPath)await fs.unlink(storedFinalPath).catch(()=>{});next(error)}})
...
app.patch('/api/assignments/:id/tasks/:taskId/contribution-section',authenticate,validate(assignmentTaskContributionSectionSchema),async(req,res,next)=>{try{
  ...
  const current=(await query('SELECT owner_id,reviewer_id FROM assignment_tasks WHERE id=$1 AND assignment_id=$2',[req.params.taskId,req.params.id])).rows[0]
  if(!current)return res.status(404).json({error:'Task not found.'})
  const manager=['Administrator','Research Manager'].includes(req.user.role)
  const {sectionKey,status,content}=req.validated
  const exactReviewer=current.reviewer_id===req.user.id
  if(!manager&&!exactReviewer&&current.owner_id!==req.user.id)return res.status(403).json({error:'Only the task owner, assigned reviewer, or a manager can update this report section.'})
  if(status==='Final'&&!manager&&!exactReviewer)return res.status(403).json({error:'Only the assigned reviewer or a manager can mark a task report section Final.'})
...
app.get('/api/dashboard',authenticate,async(req,res,next)=>{
  try{
    ...
    const reviews=await query(`
      SELECT * FROM (
        ...
        WHERE t.reviewer_id=$1
          ...
          AND t.contribution_status IN('Ready for Integration','Integrated')
    ...
    res.json({
      ...
      myWork:unique.slice(0,20),deadlines:deadlines.slice(0,20),recentActivity:activity.rows,quickActions,

...
app.post('/api/research/:id/report/generate',authenticate,validate(z.object({
  ...
  if(
    ...
    return res.status(403).json({
      error:'Only the reviewer who actually approved this submitted version can generate its final report.'
    })
  }
  ...
  if(!draftMode){
    if(!isManager&&approvedVersion.reviewed_by!==req.user.id){
      return res.status(403).json({error:'Your approval identity does not match the approved report version.'})
    }

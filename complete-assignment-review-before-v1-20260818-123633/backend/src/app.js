...
app.get('/api/generated-documents/:id',authenticate,async(req,res,next)=>{try{
  ...
  const canSubmitReport=Boolean(editableStage&&['Lead','Manager'].includes(role))
...
app.post('/api/assignment-reports/:id/submit',authenticate,validate(z.object({reviewerId:z.string().uuid(),reviewDueDate:z.string().nullable().default(null),comments:z.string().max(4000).default('')})),async(req,res,next)=>{try{
  ...
  const result=await transaction(async client=>{
    ...
    if(!imported&&(!sections.length||sections.some(item=>!['Ready','Complete'].includes(item.section_status))))throw Object.assign(new Error('Mark every required report section Ready before submission, or import a complete PDF/DOCX report for review.'),{status:409})

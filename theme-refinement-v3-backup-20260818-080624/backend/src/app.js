...
const buildTaskContributionReport=({task,assignment,ownerName,preparedBy,override={}})=>{
  const title=plainTaskReportText(override.contributionTitle??task.contribution_title)||`${task.title} â€” Contribution Report`
  ...
  *{box-sizing:border-box}body{margin:0;background:#eef1f3;color:#1f252a;font-family:Arial,Helvetica,sans-serif}
  ...
  <header class="head"><div class="eyebrow">APP2 Â· TASK CONTRIBUTION REPORT</div><h1>${escapeContributionHtml(title)}</h1><div class="sub">${escapeContributionHtml(task.title)}</div></header>
  <section class="meta">
...
if(v.ownerId){
  await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'New task contribution assigned',$2,'assignment_task',$3)",
    [v.ownerId,`You have been assigned â€œ${created.title}â€ for â€œ${assignment?.title||'an assignment'}â€.${v.expectedContribution?` Expected contribution: ${v.expectedContribution}`:''}`,created.id])
}
if(v.reviewerId){
  await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Task review assigned',$2,'assignment_task',$3)",
    [v.reviewerId,`You are the assigned reviewer for â€œ${created.title}â€ in â€œ${assignment?.title||'an assignment'}â€. The review becomes actionable when the assignee submits the task contribution.`,created.id])
}
  ...
  try{
    ...
    const result=await transaction(async client=>{
      ...
      if(v.decision==='Rejected'){
        ...
        await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) VALUES($1,'Task request declined',$2,'assignment_task_request',$3)",[request.requested_by,`Your task request â€œ${request.title}â€ was declined${v.comments?`: ${v.comments}`:'.'}`,request.id])
      ...
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) VALUES($1,'Task request approved',$2,'assignment_task_request',$3)",[request.requested_by,`Your task request â€œ${request.title}â€ was approved and converted to task â€œ${task.title}â€.`,request.id])
      if(taskOwner)await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) VALUES($1,'New task assigned',$2,'assignment_task',$3)",[taskOwner,`You have been assigned â€œ${task.title}â€.`,task.id])
...
app.patch('/api/assignments/:id/tasks/:taskId/contribution',authenticate,validate(assignmentTaskContributionSchema),async(req,res,next)=>{try{
  ...
  const row=await transaction(async client=>{
    ...
    if(v.contributionStatus==='Ready for Integration'){
      const body=`${req.user.name||current.owner_name||'A team member'} submitted the task report for â€œ${updated.title}â€ in â€œ${assignment?.title||'the assignment'}â€. Review this submitted version and approve, request changes, or reject it.`
      await client.query(
  ...
  const updated=await transaction(async client=>{
    ...
    if(row.owner_id){
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,$2,$3,'assignment_task',$4)",[
        ...
        decision==='Approved'
          ? `Your task report for â€œ${row.title}â€ was approved. The assigned reviewer will now generate the final report.`
          : decision==='Rejected'
            ? `Your task report for â€œ${row.title}â€ was rejected. Reason: ${comments}. A manager must reopen the task before further editing.`
            : `Changes were requested on â€œ${row.title}â€. ${comments}`,
        row.id
      ])
    }
    if(decision==='Rejected'){
      await client.query(`INSERT INTO notifications(user_id,title,body,entity_type,entity_id)
        ...
        WHERE u.active=TRUE AND u.id<>$4 AND (u.role IN('Administrator','Research Manager') OR am.member_role='Lead')`,[
          `â€œ${row.title}â€ was rejected by ${req.user.name||'the assigned reviewer'}. Review the reason and reopen for revision or reassign the task.`,row.id,req.params.id,req.user.id
      ])
    }
    if(decision==='Approved'){
      ...
      for(const userId of finalizers){
        await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Final task report ready to generate',$2,'assignment_task',$3)",[
          userId,`â€œ${row.title}â€ is approved. Generate the final report to complete the task.`,row.id
        ])
      }
    }
...
app.post('/api/assignments/:id/tasks/:taskId/reopen-after-rejection',authenticate,async(req,res,next)=>{try{
  ...
  const row=await transaction(async client=>{
    ...
    if(updated.owner_id)await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Rejected task reopened for revision',$2,'assignment_task',$3)",[updated.owner_id,`â€œ${updated.title}â€ has been reopened for revision. Update the draft, preview it, and resubmit to the assigned reviewer.`,updated.id])
...
app.post('/api/assignments/:id/tasks/:taskId/contribution-finalize',authenticate,async(req,res,next)=>{let storedFinalPath;try{
  ...
  const repositoryTitle=plainTaskReportText(current.contribution_title)||`${current.title} â€” Final Task Report`
  ...
  const result=await transaction(async client=>{
    ...
    const description=`Final approved task report for â€œ${current.title}â€ under assignment â€œ${current.assignment_title}â€. Prepared by ${current.owner_name||'the task owner'} and approved by ${reviewerName}.`
    ...
    if(row.owner_id){
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Task completed',$2,'assignment_task',$3)",[
        row.owner_id,`The final approved report for â€œ${row.title}â€ has been generated, published to the Document Repository, and the task is complete.`,row.id
      ])
    }
...
app.delete('/api/assignments/:id/tasks/:taskId',authenticate,authorize('Administrator','Research Manager'),validate(z.object({reason:z.string().trim().min(10).max(1000)})),async(req,res,next)=>{try{
  const result=await transaction(async client=>{
    ...
    if(task.owner_id&&task.owner_id!==req.user.id){
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Task deleted',$2,'assignment',$3)",[task.owner_id,`The task â€œ${task.title}â€ was deleted by ${req.user.name||'an authorised manager'}. Reason: ${reason}`,req.params.id])
    }
...
app.post('/api/assignments/:id/reviews',authenticate,validate(z.object({decision:z.enum(['Submitted','Under Review','Changes Requested','Approved']),comments:z.string().max(4000).default('')})),async(req,res,next)=>{
  try{
    ...
    const row=await transaction(async client=>{
      ...
      if(!allowed[req.validated.decision].includes(latest))throw Object.assign(new Error(`The â€œ${req.validated.decision}â€ action is not valid after ${latest||'no submission'}. Refresh the Review tab.`),{status:409})
...
const reportPlainText=value=>String(value||'').replace(/<br\s*\/?>/gi,'\n').replace(/<\/(td|th)>/gi,'\t').replace(/<\/tr>/gi,'\n').replace(/<\/p>|<\/li>|<\/h[1-6]>/gi,'\n').replace(/<li[^>]*>/gi,'â€¢ ').replace(/<[^>]+>/g,'').replace(/&nbsp;/gi,' ').replace(/&amp;/gi,'&').replace(/&lt;/gi,'<').replace(/&gt;/gi,'>').replace(/&quot;/gi,'"').replace(/&#39;/gi,"'").replace(/[ \t]+\n/g,'\n').replace(/\n{3,}/g,'\n\n').trim()
...
const compileAssignmentSectionContent=({section,tasks,assignment,knowledge,reportType})=>{
  ...
  const taskTitles=tasks.map(task=>`<li>${escapeHtml(task.contribution_title||task.title)} â€” ${escapeHtml(task.owner_name||'Task owner')}</li>`).join('')
  ...
  if(key==='references'){const taskRefs=tasks.map(task=>`<li>Final Task Report: ${escapeHtml(task.contribution_title||task.title)} â€” ${escapeHtml(task.owner_name||'Task owner')}</li>`).join('');const repoRefs=knowledge.map(item=>`<li>Repository evidence: ${escapeHtml(item.title)} Â· ${escapeHtml(item.category||'Document')}</li>`).join('');return taskRefs||repoRefs?`<ul>${taskRefs}${repoRefs}</ul>`:''}
  if(key==='appendices'){const taskRefs=tasks.map((task,index)=>`<li>Appendix ${index+1}: Final Task Report â€” ${escapeHtml(task.contribution_title||task.title)} (${escapeHtml(task.owner_name||'Task owner')})</li>`).join('');const repoRefs=knowledge.map((item,index)=>`<li>Supporting Evidence ${index+1}: ${escapeHtml(item.title)} Â· ${escapeHtml(item.category||'Document')}</li>`).join('');return taskRefs||repoRefs?`<p>This source register records the approved inputs used to compile the Final Assignment Report.</p><ul>${taskRefs}${repoRefs}</ul>`:'<p>No appendices or supporting sources have been registered.</p>'}
...
app.post('/api/assignment-reports/:id/submit',authenticate,validate(z.object({reviewerId:z.string().uuid(),reviewDueDate:z.string().nullable().default(null),comments:z.string().max(4000).default('')})),async(req,res,next)=>{try{
  ...
  const result=await transaction(async client=>{
    ...
    await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Assignment report review assigned',$2,'generated_document',$3)",[reviewer.id,`Review â€œ${document.title}â€ for assignment â€œ${document.context_id}â€.`,document.id])
...
app.post('/api/assignment-reports/:id/review',authenticate,validate(z.object({decision:z.enum(['Under Review','Changes Requested','Approved']),comments:z.string().max(4000).default('')})),async(req,res,next)=>{try{
  ...
  const updated=await transaction(async client=>{
    ...
    await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,$2,$3,'generated_document',$4)",[document.created_by,req.validated.decision==='Changes Requested'?'Assignment report changes requested':'Assignment report approved',`â€œ${document.title}â€ is now ${req.validated.decision}. ${req.validated.comments}`,'generated_document',document.id])
...
app.post('/api/assignment-reports/:id/finalize',authenticate,async(req,res,next)=>{let storedFinalPath;try{
  ...
  const repositoryTitle=document.title.replace(/\s*[â€”-]\s*Draft Assignment Report$/i,'').trim()||`${document.assignment_title} â€” Final Assignment Report`
  const finalSections=[...sections.filter(section=>section.section_key!=='review_approval'),{title:'Review and Approval Record',content:`Status: FINAL â€” APPROVED
...
Controlled report version: ${document.version}`}]
  ...
  const finalDocx=await buildDocx(repositoryTitle,{Organization:'Public Service Commission','Report Type':'Final Assignment Report','Assignment Reference':assignmentReference,'Assignment Title':document.assignment_title,Division:document.assignment_division||'Not specified','Assignment Lead':document.lead_name||'Not specified','Team Members':document.team_names||document.lead_name||'Not specified','Reporting Period':`${reportingStart} â€“ ${reportingEnd}`,'Report Reference':document.reference,'Controlled Version':String(document.version),Classification:document.classification||'Official','Prepared / Compiled by':document.created_by_name,'Reviewed and Approved by':document.reviewer_name||req.user.name,'Approval Date':approvalTime,Status:'FINAL â€” APPROVED'},finalSections)
  ...
  const result=await transaction(async client=>{
    ...
    const category=(await client.query("SELECT id FROM document_categories WHERE name='Assignment Reports' LIMIT 1")).rows[0]||null
    const description=`Final approved assignment report for â€œ${document.assignment_title}â€. Prepared by ${document.created_by_name} and approved by ${document.reviewer_name||req.user.name}.`
    ...
    await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Final Assignment Report published',$2,'generated_document',$3)",[document.created_by,`The Final Assignment Report for â€œ${document.assignment_title}â€ has been generated, published to the Document Repository, and the assignment is complete.`,document.id])

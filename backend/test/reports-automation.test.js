...
test('reporting frontend is lazy loaded and exposes final workflow controls',()=>{assert.match(shell,/lazy\(\(\)\s*=>\s*import\(["']\.\/ReportsModule["']\)\)/);for(const marker of ['Schedule','Decision','Sign off','Configure'])assert.ok(frontend.includes(marker))})

...
export default function AssignmentReportBuilder({
  ...
  const save = async (markReady = false) => {
    ...
    if (!dirty && !markReady) return true;
    setSaving(true);
    try {
      const plain = content.replace(/<[^>]+>/g, " ").trim();
      if (markReady && !plain) throw new Error("Add content before marking this section Ready.");
  ...
  return (
    <TaskSectionWorkspace
      ...
      canMarkReady={editable && Boolean(content.replace(/<[^>]+>/g, " ").trim())}

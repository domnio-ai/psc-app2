...
const buildTaskContributionReport=({task,assignment,ownerName,preparedBy,override={}})=>{
  const title=plainTaskReportText(override.contributionTitle??task.contribution_title)||`${task.title} Ã¢â‚¬” Contribution Report`
...
if(v.ownerId){
  await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'New task contribution assigned',$2,'assignment_task',$3)",
    [v.ownerId,`You have been assigned “${created.title}Ã¢â‚¬Â for “${assignment?.title||'an assignment'}Ã¢â‚¬Â.${v.expectedContribution?` Expected contribution: ${v.expectedContribution}`:''}`,created.id])
}
if(v.reviewerId){
  await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Task review assigned',$2,'assignment_task',$3)",
    [v.reviewerId,`You are the assigned reviewer for “${created.title}Ã¢â‚¬Â in “${assignment?.title||'an assignment'}Ã¢â‚¬Â. The review becomes actionable when the assignee submits the task contribution.`,created.id])
}
  ...
  try{
    ...
    const result=await transaction(async client=>{
      ...
      if(v.decision==='Rejected'){
        ...
        await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) VALUES($1,'Task request declined',$2,'assignment_task_request',$3)",[request.requested_by,`Your task request “${request.title}Ã¢â‚¬Â was declined${v.comments?`: ${v.comments}`:'.'}`,request.id])
      ...
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) VALUES($1,'Task request approved',$2,'assignment_task_request',$3)",[request.requested_by,`Your task request “${request.title}Ã¢â‚¬Â was approved and converted to task “${task.title}Ã¢â‚¬Â.`,request.id])
      if(taskOwner)await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) VALUES($1,'New task assigned',$2,'assignment_task',$3)",[taskOwner,`You have been assigned “${task.title}Ã¢â‚¬Â.`,task.id])
...
app.patch('/api/assignments/:id/tasks/:taskId/contribution',authenticate,validate(assignmentTaskContributionSchema),async(req,res,next)=>{try{
  ...
  const row=await transaction(async client=>{
    ...
    if(v.contributionStatus==='Ready for Integration'){
      const body=`${req.user.name||current.owner_name||'A team member'} submitted the task report for “${updated.title}Ã¢â‚¬Â in “${assignment?.title||'the assignment'}Ã¢â‚¬Â. Review this submitted version and approve, request changes, or reject it.`
      await client.query(
  ...
  const updated=await transaction(async client=>{
    ...
    if(row.owner_id){
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,$2,$3,'assignment_task',$4)",[
        ...
        decision==='Approved'
          ? `Your task report for “${row.title}Ã¢â‚¬Â was approved. The assigned reviewer will now generate the final report.`
          : decision==='Rejected'
            ? `Your task report for “${row.title}Ã¢â‚¬Â was rejected. Reason: ${comments}. A manager must reopen the task before further editing.`
            : `Changes were requested on “${row.title}Ã¢â‚¬Â. ${comments}`,
        row.id
      ])
    }
    if(decision==='Rejected'){
      await client.query(`INSERT INTO notifications(user_id,title,body,entity_type,entity_id)
        ...
        WHERE u.active=TRUE AND u.id<>$4 AND (u.role IN('Administrator','Research Manager') OR am.member_role='Lead')`,[
          `“${row.title}Ã¢â‚¬Â was rejected by ${req.user.name||'the assigned reviewer'}. Review the reason and reopen for revision or reassign the task.`,row.id,req.params.id,req.user.id
      ])
    }
    if(decision==='Approved'){
      ...
      for(const userId of finalizers){
        await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Final task report ready to generate',$2,'assignment_task',$3)",[
          userId,`“${row.title}Ã¢â‚¬Â is approved. Generate the final report to complete the task.`,row.id
        ])
      }
    }
...
app.post('/api/assignments/:id/tasks/:taskId/reopen-after-rejection',authenticate,async(req,res,next)=>{try{
  ...
  const row=await transaction(async client=>{
    ...
    if(updated.owner_id)await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Rejected task reopened for revision',$2,'assignment_task',$3)",[updated.owner_id,`“${updated.title}Ã¢â‚¬Â has been reopened for revision. Update the draft, preview it, and resubmit to the assigned reviewer.`,updated.id])
...
app.post('/api/assignments/:id/tasks/:taskId/contribution-finalize',authenticate,async(req,res,next)=>{let storedFinalPath;try{
  ...
  const repositoryTitle=plainTaskReportText(current.contribution_title)||`${current.title} Ã¢â‚¬” Final Task Report`
  ...
  const result=await transaction(async client=>{
    ...
    const description=`Final approved task report for “${current.title}Ã¢â‚¬Â under assignment “${current.assignment_title}Ã¢â‚¬Â. Prepared by ${current.owner_name||'the task owner'} and approved by ${reviewerName}.`
    ...
    if(row.owner_id){
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Task completed',$2,'assignment_task',$3)",[
        row.owner_id,`The final approved report for “${row.title}Ã¢â‚¬Â has been generated, published to the Document Repository, and the task is complete.`,row.id
      ])
    }
...
app.delete('/api/assignments/:id/tasks/:taskId',authenticate,authorize('Administrator','Research Manager'),validate(z.object({reason:z.string().trim().min(10).max(1000)})),async(req,res,next)=>{try{
  const result=await transaction(async client=>{
    ...
    if(task.owner_id&&task.owner_id!==req.user.id){
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Task deleted',$2,'assignment',$3)",[task.owner_id,`The task “${task.title}Ã¢â‚¬Â was deleted by ${req.user.name||'an authorised manager'}. Reason: ${reason}`,req.params.id])
    }
...
app.post('/api/assignments/:id/reviews',authenticate,validate(z.object({decision:z.enum(['Submitted','Under Review','Changes Requested','Approved']),comments:z.string().max(4000).default('')})),async(req,res,next)=>{
  try{
    ...
    const row=await transaction(async client=>{
      ...
      if(!allowed[req.validated.decision].includes(latest))throw Object.assign(new Error(`The “${req.validated.decision}Ã¢â‚¬Â action is not valid after ${latest||'no submission'}. Refresh the Review tab.`),{status:409})
...
const compileAssignmentSectionContent=({section,tasks,assignment,knowledge,reportType})=>{
  ...
  const taskTitles=tasks.map(task=>`<li>${escapeHtml(task.contribution_title||task.title)} Ã¢â‚¬” ${escapeHtml(task.owner_name||'Task owner')}</li>`).join('')
...
app.post('/api/assignment-reports/:id/submit',authenticate,validate(z.object({reviewerId:z.string().uuid(),reviewDueDate:z.string().nullable().default(null),comments:z.string().max(4000).default('')})),async(req,res,next)=>{try{
  ...
  const result=await transaction(async client=>{
    ...
    await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Assignment report review assigned',$2,'generated_document',$3)",[reviewer.id,`Review “${document.title}Ã¢â‚¬Â for assignment “${document.context_id}Ã¢â‚¬Â.`,document.id])
...
app.post('/api/assignment-reports/:id/review',authenticate,validate(z.object({decision:z.enum(['Under Review','Changes Requested','Approved']),comments:z.string().max(4000).default('')})),async(req,res,next)=>{try{
  ...
  const updated=await transaction(async client=>{
    ...
    await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,$2,$3,'generated_document',$4)",[document.created_by,req.validated.decision==='Changes Requested'?'Assignment report changes requested':'Assignment report approved',`“${document.title}Ã¢â‚¬Â is now ${req.validated.decision}. ${req.validated.comments}`,'generated_document',document.id])
...
app.post('/api/assignment-reports/:id/finalize',authenticate,async(req,res,next)=>{let storedFinalPath;try{
  ...
  const repositoryTitle=document.title.replace(/\s*[Ã¢â‚¬”-]\s*Draft Assignment Report$/i,'').trim()||`${document.assignment_title} Ã¢â‚¬” Final Assignment Report`
  const finalSections=[...sections.filter(section=>section.section_key!=='review_approval'),{title:'Review and Approval Record',content:`Status: FINAL Ã¢â‚¬” APPROVED
...
Controlled report version: ${document.version}`}]
  ...
  const finalDocx=await buildDocx(repositoryTitle,{Organization:'Public Service Commission','Report Type':'Final Assignment Report','Assignment Reference':assignmentReference,'Assignment Title':document.assignment_title,Division:document.assignment_division||'Not specified','Assignment Lead':document.lead_name||'Not specified','Team Members':document.team_names||document.lead_name||'Not specified','Reporting Period':`${reportingStart} – ${reportingEnd}`,'Report Reference':document.reference,'Controlled Version':String(document.version),Classification:document.classification||'Official','Prepared / Compiled by':document.created_by_name,'Reviewed and Approved by':document.reviewer_name||req.user.name,'Approval Date':approvalTime,Status:'FINAL Ã¢â‚¬” APPROVED'},finalSections)
  ...
  const result=await transaction(async client=>{
    ...
    const category=(await client.query("SELECT id FROM document_categories WHERE name='Assignment Reports' LIMIT 1")).rows[0]||null
    const description=`Final approved assignment report for “${document.assignment_title}Ã¢â‚¬Â. Prepared by ${document.created_by_name} and approved by ${document.reviewer_name||req.user.name}.`
    ...
    await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Final Assignment Report published',$2,'generated_document',$3)",[document.created_by,`The Final Assignment Report for “${document.assignment_title}Ã¢â‚¬Â has been generated, published to the Document Repository, and the assignment is complete.`,document.id])
...
app.post('/api/document-reviews/:id/decision',authenticate,authorize('Administrator','Research Manager','Reviewer'),validate(z.object({approved:z.boolean(),comments:z.string().max(2000).default('')})),async(req,res,next)=>{
  try{
    ...
    const result=await transaction(async client=>{
      ...
      await client.query('INSERT INTO notifications(user_id,title,body,entity_type,entity_id) VALUES($1,$2,$3,$4,$5)',[item.created_by,req.validated.approved?'Document approved':'Document returned for correction',req.validated.approved?`“${item.title}Ã¢â‚¬Â has been approved and published.`:`“${item.title}Ã¢â‚¬Â was rejected: ${req.validated.comments}`,'knowledge',item.id])
...
app.get('/api/notifications',authenticate,async(req,res,next)=>{
  try{
    await transaction(async client=>{
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) SELECT am.user_id,'Assigned task','You are assigned to “'||a.title||'Ã¢â‚¬Â ('||a.status||')'||CASE WHEN a.due_date IS NOT NULL THEN ', due '||TO_CHAR(a.due_date,'DD Mon YYYY') ELSE '' END||'.','assignment',a.id FROM assignments a JOIN assignment_members am ON am.assignment_id=a.id WHERE am.user_id=$1 AND NOT EXISTS(SELECT 1 FROM notifications n WHERE n.user_id=am.user_id AND n.entity_id=a.id::text AND n.title IN('Assigned task','New assignment','Assignment allocated'))",[req.user.id])
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) SELECT am.user_id,'Assignment status update','“'||a.title||'Ã¢â‚¬Â is now '||a.status||'.','assignment_status',a.id FROM assignments a JOIN assignment_members am ON am.assignment_id=a.id WHERE am.user_id=$1 AND NOT EXISTS(SELECT 1 FROM notifications n WHERE n.user_id=am.user_id AND n.entity_id=a.id::text AND n.title='Assignment status update' AND n.body='“'||a.title||'Ã¢â‚¬Â is now '||a.status||'.')",[req.user.id])
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) SELECT am.user_id,'Assignment completed','“'||a.title||'Ã¢â‚¬Â has been completed and retained in your activity history.','assignment_completed',a.id FROM assignments a JOIN assignment_members am ON am.assignment_id=a.id WHERE am.user_id=$1 AND a.status='Completed' AND NOT EXISTS(SELECT 1 FROM notifications n WHERE n.user_id=am.user_id AND n.entity_id=a.id::text AND n.title='Assignment completed')",[req.user.id])
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) SELECT am.user_id,'Assignment due soon','“'||a.title||'Ã¢â‚¬Â is due on '||TO_CHAR(a.due_date,'DD Mon YYYY')||'. Please review and update its progress.','assignment_due',a.id FROM assignments a JOIN assignment_members am ON am.assignment_id=a.id WHERE am.user_id=$1 AND a.status NOT IN('Completed','Overdue') AND a.due_date BETWEEN CURRENT_DATE AND CURRENT_DATE+2 AND NOT EXISTS(SELECT 1 FROM notifications n WHERE n.user_id=am.user_id AND n.entity_id=a.id::text AND n.title='Assignment due soon' AND n.body LIKE '%'||TO_CHAR(a.due_date,'DD Mon YYYY')||'%')",[req.user.id])
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) SELECT am.user_id,'Assignment overdue','“'||a.title||'Ã¢â‚¬Â was due on '||TO_CHAR(a.due_date,'DD Mon YYYY')||'. Immediate action is required.','assignment_overdue',a.id FROM assignments a JOIN assignment_members am ON am.assignment_id=a.id WHERE am.user_id=$1 AND a.status!='Completed' AND a.due_date<CURRENT_DATE AND NOT EXISTS(SELECT 1 FROM notifications n WHERE n.user_id=am.user_id AND n.entity_id=a.id::text AND n.title='Assignment overdue' AND n.body LIKE '%'||TO_CHAR(a.due_date,'DD Mon YYYY')||'%')",[req.user.id])
      await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id) SELECT $1,'Previously approved notice','“'||a.title||'Ã¢â‚¬Â was approved and published on the Notice Board.','notice',a.id FROM alerts a WHERE a.status='Published' AND(a.audience_role IS NULL OR a.audience_role=$2) AND NOT EXISTS(SELECT 1 FROM notifications n WHERE n.user_id=$1 AND n.entity_type='notice' AND n.entity_id=a.id::text)",[req.user.id,req.user.role])
    })

            ...
            <article key={report.id} className="assignment-report-workflow-card">
              ...
              <div className="assignment-report-next-action">
                <div>
                  ...
                  <span>{report.status === "Approved" ? "Generate and publish the approved final report." : sectionsReady ? "Preview the completed draft, assign a reviewer and submit it." : `Complete and mark ${sectionsRemaining} remaining section${sectionsRemaining === 1 ? "" : "s"} Ready.`}</span>
                </div>
                <b>{ready}/{total} Ready</b>
              </div>
              <footer>
                ...
                {canManage && ["Draft", "Revised", "Changes Requested"].includes(report.status) && (
                  <div className="assignment-report-submit-controls">
                    ...
                    {!sectionsReady && (
                      <small className="assignment-report-submit-blocker">Mark all {total} sections Ready before submission ({sectionsRemaining} remaining).</small>
                    )}
                  </div>
                )}

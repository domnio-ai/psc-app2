...
export default function ExternalResearchImportModal({ token, users, onClose, onCreated }: Props) {
  ...
  return <div className="modal-backdrop external-research-import-backdrop" onClick={onClose}>
    <form className="external-research-import-modal" onSubmit={submit} onClick={(event) => event.stopPropagation()}>
      <header><div><p>EXTERNAL COMPLETED RESEARCH</p><h2>Import for review</h2><span>This import leaves the research workspace. A formal reviewer is required before the imported research can become an approved final institutional report.</span></div><button type="button" className="external-close" onClick={onClose}>×</button></header>
      ...
      <section className="external-reviewer-picker"><div><strong>Final report reviewer(s) *</strong><span>The import cannot proceed without a reviewer. Approval is required before the file becomes the final institutional research report.</span></div><div>{reviewers.map((person) => <label key={person.id} className={form.reviewerIds.includes(person.id) ? "selected" : ""}><input type="checkbox" checked={form.reviewerIds.includes(person.id)} onChange={() => toggleReviewer(person.id)}/><span><strong>{person.name}</strong><small>{person.role} · {person.division}</small></span></label>)}</div></section>
      ...
      <footer><button type="button" className="external-secondary" onClick={onClose}>Cancel</button><button type="submit" className="external-primary" disabled={busy || !file || !form.title.trim() || !form.reviewerIds.length}>{busy ? "Importing…" : "Import & send for review"}</button></footer>
    </form>
  </div>;
}

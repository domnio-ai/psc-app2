...
const assignmentFinalReportSections=[
  ...
  {key:'background',title:'Background / Assignment Brief'},
  {key:'objectives',title:'Objectives'},
  {key:'work_undertaken',title:'Work Undertaken'},
  {key:'evidence_reviewed',title:'Evidence Reviewed'},
  {key:'key_findings',title:'Key Findings'},
  {key:'gaps_risks',title:'Gaps / Risks / Challenges'},
  {key:'recommendations',title:'Recommendations'},
  {key:'action_matrix',title:'Action Matrix / Next Actions'},
  {key:'conclusion',title:'Conclusion'},
  {key:'references',title:'References / Supporting Evidence'},
...
const ensureAssignmentFinalReportTemplate=async()=>{
  await query(`INSERT INTO document_templates(template_key,name,context,description,sections,version,governance_status,active)
    VALUES('assignment-final-report','Final Assignment Report','Assignment','Controlled final assignment report compiled from approved task reports and published evidence.',$1::jsonb,1,'Standard',TRUE)
    ON CONFLICT(template_key) DO NOTHING`,[JSON.stringify(assignmentFinalReportSections)])
...
const compiledTaskBlocks=(tasks,field,label)=>tasks.map(task=>{const value=String(task[field]||'').trim();if(!value)return '';return `<article data-source-task-id="${task.id}"><h3>${escapeHtml(task.contribution_title||task.title)}</h3><p><strong>Prepared by:</strong> ${escapeHtml(task.owner_name||'Task owner')}</p><h4>${escapeHtml(label)}</h4>${compiledParagraph(value)}</article>`}).filter(Boolean).join('<hr>')
const compileAssignmentSectionContent=({section,tasks,assignment,knowledge,reportType})=>{
  ...
  if(key==='background')return compiledParagraph(assignment.description||`Assignment: ${assignment.title}`)
  if(['objectives','assignment_objectives'].includes(key))return compiledParagraph(assignment.description||'Complete the approved assignment work and deliver the required institutional outputs.')
  if(['work_undertaken','activities_undertaken','outputs'].includes(key))return compiledTaskBlocks(tasks,'contribution_summary','Work completed')
  ...
  if(['action_matrix','next_steps','next-steps'].includes(key))return compiledTaskBlocks(tasks,'contribution_next_actions','Next actions')
  ...
  if(key==='conclusion')return `<p>The assignment report has been compiled from ${accepted}. The Assignment Lead should verify completeness, consistency and institutional actions before formal review.</p>`
  if(['references','appendices'].includes(key)){const taskRefs=tasks.map(task=>`<li>Final Task Report: ${escapeHtml(task.contribution_title||task.title)} — ${escapeHtml(task.owner_name||'Task owner')}</li>`).join('');const repoRefs=knowledge.map(item=>`<li>Repository: ${escapeHtml(item.title)} · ${escapeHtml(item.category||'Document')}</li>`).join('');return taskRefs||repoRefs?`<ul>${taskRefs}${repoRefs}</ul>`:''}
...
app.post('/api/assignment-reports/:id/submit',authenticate,validate(z.object({reviewerId:z.string().uuid(),reviewDueDate:z.string().nullable().default(null),comments:z.string().max(4000).default('')})),async(req,res,next)=>{try{
  ...
  const result=await transaction(async client=>{
    ...
    await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Assignment report review assigned',$2,'generated_document',$3)",[reviewer.id,`Review “${document.title}” for assignment “${document.context_id}”.`,document.id])
...
app.post('/api/assignment-reports/:id/review',authenticate,validate(z.object({decision:z.enum(['Under Review','Changes Requested','Approved']),comments:z.string().max(4000).default('')})),async(req,res,next)=>{try{
  ...
  const updated=await transaction(async client=>{
    ...
    await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,$2,$3,'generated_document',$4)",[document.created_by,req.validated.decision==='Changes Requested'?'Assignment report changes requested':'Assignment report approved',`“${document.title}” is now ${req.validated.decision}. ${req.validated.comments}`,'generated_document',document.id])
...
Controlled report version: ${document.version}`}]
  const finalDocx=await buildDocx(repositoryTitle,{Assignment:document.assignment_title,Division:document.assignment_division||'',Reference:document.reference,'Assignment Lead':document.lead_name||'Not specified','Prepared by':document.created_by_name,'Reviewed and approved by':document.reviewer_name||req.user.name,Status:'FINAL — APPROVED',Version:document.version,'Approval date':approvalTime},finalSections)
  ...
  const result=await transaction(async client=>{
    ...
    const category=(await client.query("SELECT id FROM document_categories WHERE name='Assignment Reports' LIMIT 1")).rows[0]||null
    const description=`Final approved assignment report for “${document.assignment_title}”. Prepared by ${document.created_by_name} and approved by ${document.reviewer_name||req.user.name}.`
    ...
    await client.query("INSERT INTO notifications(user_id,title,body,entity_type,entity_id)VALUES($1,'Final Assignment Report published',$2,'generated_document',$3)",[document.created_by,`The Final Assignment Report for “${document.assignment_title}” has been generated, published to the Document Repository, and the assignment is complete.`,document.id])
